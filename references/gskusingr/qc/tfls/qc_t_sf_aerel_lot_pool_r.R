####------------------------------------------------------------------------####
# Program Name: qc_t_sf_aerel_lot_pool_r.R
# Domino Environment: Multilingual Frozen R4.5.1_2025-09-09_v1 SASViya4
# Developer: Pragya Tripathi / ps209009
# Purpose: Summary of IDRX-42-related Treatment-Emergent Adverse Events by 
#          System Organ Class, Preferred Term, line of therapy, and LOTCAT 
#          subgroup (Phase 1 & 1b pooled)
#
# Input:
#   - adam [adbs, adae]
# Output:
...
# --- Load Libraries ---
libs <- c("dplyr", "forcats", "stringr", "purrr", "cards", "tfrmt", "gt", 
...
prod <- read_parquet(file.path(g_paths$dddata, "t_sf_aerel_lot_pool_r.parquet")) |>
...
qc_result <- diffdf(
  ...
  file = file.path(g_paths$qc_output, "qc_t_sf_aerel_lot_pool_r.lst")
)
