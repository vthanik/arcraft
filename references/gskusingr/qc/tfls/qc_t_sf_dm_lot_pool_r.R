####------------------------------------------------------------------------####
# Program Name: qc_t_sf_dm_lot_pool_r.R
...
ard_n <- ard_tabulate(data = adbs,
  ...
  unlist_ard_columns() %>%
  select(dosgrp=variable_level,stat,lotcat=group1_level,stat_label,stat_name) %>%
  mutate(
    stat_variable = "STUDYID",
    label = str_replace_all(stat_label, "n", "N"),
    stat_name = str_replace_all(stat_name, "n", "BigN"),
    ord1 = 10
  )

#removing missing values 
adbs_ <- adbs %>% filter(!is.na(dosgrp) & !is.na(lotcat))
...
qc <- 
  select(
    ard_cont_,
    dosgrp, lotcat, stat_variable, label, stat_name, stat,ord1
  ) %>%
  bind_rows(
    select(
      ard_cat_,
...
prod <- read_parquet(file.path(g_paths$dddata, "t_sf_dm_lot_pool_r.parquet")) %>%
  select(-ord2,-order) %>%
  rename_with(tolower) 
...
diffdf::diffdf(base = prod ,
                    compare = qc,
                    keys = c('dosgrp', 'lotcat','stat_name','label','ord1')
               )
