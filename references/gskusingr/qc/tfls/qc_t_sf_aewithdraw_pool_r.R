####------------------------------------------------------------------------####
# Program Name: qc_t_sf_aewithdraw_pool_r.R
# Domino Environment: Multilingual Frozen R4.5.1_2025-09-09_v1 SASViya4
# Developer: Pragya Tripathi / ps209009
# Purpose: Summary of Treatment-Emergent Adverse Events Leading to IDRX-42 Withdrawn
# by System Organ Class and Preferred Term (Phase 1 & 1b pooled)
#
# Input:
#   - adam [adbs, adae]
# Output:
...
adae_1 <- convert_blanks_to_na(adae)|>
  filter(SAFFL == "Y" & (AEACN == "DRUG WITHDRAWN" |
                           ...
                           AEACN3 == "DRUG WITHDRAWN") & TRTEMFL == "Y") |>
...
prod <- read_parquet(file.path(g_paths$dddata,"t_sf_aewithdraw_pool_r.parquet")) |>
  filter(stat != "0") |>
  arrange(DOSGRP, variable, variable_level, stat_name)
...
qc_result <- diffdf(base = prod ,
                    ...
                    file = file.path(g_paths$qc_output, "qc_t_sf_aewithdraw_pool_r.lst")
)
