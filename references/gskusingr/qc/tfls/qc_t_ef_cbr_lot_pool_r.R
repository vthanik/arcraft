####------------------------------------------------------------------------####
# Program Name: qc_t_ef_cbr_lot_pool_r.R
...
# Purpose: To create Table Table 14.2.6e  -  Summary of Clinical Benefit Rate by Line of Therapy (Phase 1 & 1b pooled)  -  Efficacy Analysis Set
# Input: 
...
ard_n <- ard_tabulate(data = adbs,
  ...
  unlist_ard_columns() %>%
  select(dosgrp=variable_level,stat,lotcat=group1_level,label=stat_label,stat_name) %>%
  mutate(
    stat_variable = "bignn",
...
ard_bor1 <- ard_tabulate(
  ...
  unlist_ard_columns() %>%
  select(dosgrp = group1_level,stat,stat_name,label=variable_level,lotcat = group2_level) %>%
  mutate(
...
prod <- read_parquet(file.path(g_paths$dddata,"t_ef_cbr_lot_pool_r.parquet")) %>%
  rename_with(tolower) %>%
  select(-order)
...
diffdf::diffdf(base = prod ,
                    compare = qc,
                    keys = c('dosgrp', 'lotcat', 'label', 'stat_name')
#                    file = file.path(g_paths$qc_output, "qc_t_ef_cbr_lot_pool_r.lst")
)
