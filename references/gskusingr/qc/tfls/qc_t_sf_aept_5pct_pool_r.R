####------------------------------------------------------------------------####
# Program Name: qc_t_sf_aept_5pct_pool_r.R
# Domino Environment: Multilingual Frozen R4.5.1_2025-09-09_v1 SASViya4
# Developer: Shilpa Kolli / sk575206
# Purpose: Summary of Treatment-Emergent Serious Adverse Events >=5% Subjects by System Organ Class 
#          and Preferred Term (Phase 1 & 1b pooled)
#
# Input:
#   - adam [adbs, adae]
# Output:
#   - qc/output/qc_t_sf_aept_5pct_pool_r.Rout
#
...
# Use filter_ard_hierarchical for post-filtering ---
# This function filters the hierarchical ARD based on a condition.
# We keep PTs where the overall percentage (p_overall) for "Any Grade" is >= 5%.
# We also explicitly keep the "Any TEAE" rows.
ard_data <- ae_ard1 |>
  filter_ard_hierarchical(
    filter = p_overall >= 0.05,
    var = AEDECOD
  ) 
...
prod_raw <- read_data_catalog(dddata$t_sf_aept_5pct_pool_r) |> 
