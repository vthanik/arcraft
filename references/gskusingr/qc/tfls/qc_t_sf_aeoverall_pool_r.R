####------------------------------------------------------------------------####
# Program Name: qc_t_sf_aeoverall_pool_r.R
...
#   - ADaM: ADBS, ADAE
# Output:
#   - - qc/output/qc_t_sf_aeoverall_pool_r.Rout
#
...
qc_summary_data <- df_numer |>
  ...
  filter(LOTCAT %in% c("1st-line", "2nd-line")) |> 
  tidyr::pivot_longer(
    cols = c("n", "p"),
    names_to = "stat_name",
    values_to = "stat"
...
prod <- read_parquet(file.path(g_paths$dddata,"t_sf_aeoverall_pool_r.parquet")) |> 
