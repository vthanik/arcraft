####------------------------------------------------------------------------####
# Program Name: qc_t_ef_orr_muttstt2_pool_r.R
...
#          by Mutation Status  in 2nd-line Therapy 
#
# Input:
#   - adam [adbs, adcm]
# Output:
...
qc <-  bind_rows(final_ard, Bign |> unlist_ard_columns()) |> filter(stat_name != "N") |>
  ...
  arrange(ARM, variable, mutstt, label, stat_name)  |> filter(stat != 0)

# --- 7. Production ---

# --- Load production data and compare ---

prod <- read_parquet(file.path(g_paths$dddata,"t_ef_orr_muttstt2_pool_r.parquet")) |>
  arrange(ARM, variable, mutstt, label, stat_name)  |> filter(stat != 0)
