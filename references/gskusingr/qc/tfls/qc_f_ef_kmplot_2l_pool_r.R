####------------------------------------------------------------------------####
# Program Name: qc_f_ef_kmplot_2l_pool_r.R
...
#   - adamdata: [N/A]
#   - dddata: [N/A]
#   - file: qc_f_ef_kmplot_2l_pool_r.lst
#
...
# Load Libraries
libs <- c("admiral","dplyr", "survival", "survminer", "ggplot2")
pkg_attach(libs)
...
adbs_1 <- convert_blanks_to_na(adbs) |>
  filter(SAFFL == "Y" & DTHFL=="Y" & LOTCATN == 1) 
...
adtte_pfs <- adtte |> 
  filter(PARAMCD == "PFS" & ACAT2 == "INVESTIGATOR" & LOTCATN == 2) |> 
