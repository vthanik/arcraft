####------------------------------------------------------------------------####
# Program Name: qc_t_ef_cbr_uprtki3_pool_r
...
# Purpose: QC t_ef_cbr_uprtki3_pool_r
# Input: ADSL ADEFF
# Output: [N/A]
...
adtteOORU0 <- read_data_catalog(adamdata$adeff) |>
  filter(
    EFFL == "Y" &
      PARAMCD == "BORU" &
      ACAT2 == "INVESTIGATOR" &
      AVALC %in% c("PR", "CR", "6 <= SD < 16 weeks", "SD >= 16 weeks")
  ) |>
  mutate(AVALC_U = "Y") |>
...
#Subjects with Prior Ripretinib
adcm01 <- read_data_catalog(adamdata$adcm) |>
  filter(CMCAT == "PRIOR CANCER THERAPY", CMTRT == "RIPRETINIB") |>
  distinct(USUBJID) |>
  mutate(REMFL = "Y")

#Excluding Bezuclastinib, NB003, and THE-630
adcm02 <- read_data_catalog(adamdata$adcm) |>
  filter(
    CMCAT == "PRIOR CANCER THERAPY" &
      str_detect(
        CMTRT,
        regex("BEZUCLASTINIB|NB003|THE-630", ignore_case = TRUE)
      )
  ) |>
  distinct(USUBJID) |>
  mutate(EXFL = "Y")

adcm <- merge(adcm01, adcm02, by = "USUBJID", all = TRUE) |>
  mutate(REMFL = case_when(EXFL == "Y" ~ NA_character_, TRUE ~ REMFL))
...
adslt <- adslt |>
  filter(!is.na(REMFL))
adtteBOR <- merge(adtte0, adslt, by = "USUBJID", all.y = TRUE)
...
t_ef_cbr_uprtki3_pool_r <- read_parquet(file.path(
  g_paths$dddata,
  "t_ef_cbr_uprtki3_pool_r.parquet"
))

#
qc_t_ef_cbr_uprtki3_pool_r <- ard_tbl1 |>
...
t_ef_cbr_uprtki3_pool_r <- t_ef_cbr_uprtki3_pool_r |>
  arrange(column, stat_name, LABEL, GROUP) |>
  select(-ord_2)

# Compare PRODUCTION vs QC datasets ----
diffdf::diffdf(
  base = t_ef_cbr_uprtki3_pool_r,
  compare = qc_t_ef_cbr_uprtki3_pool_r
)