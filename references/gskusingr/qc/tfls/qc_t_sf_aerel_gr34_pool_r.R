####------------------------------------------------------------------------####
# Program Name: qc_t_sf_aerel_gr34_pool_r.R
# Domino Environment: Multilingual Frozen R4.5.1_2025-09-09_v1 SASViya4
# Developer: Pragya Tripathi / ps209009
# Purpose: Summary of IDRX-42-related Treatment-Emergent Adverse Events with CTCAE Grade 3 or 4 
...
adae_1 <- convert_blanks_to_na(adae)|>
  filter(SAFFL == "Y" & TRTEMFL == "Y" & AREL=="Related"
           & (ATOXGRN == 3 |ATOXGRN == 4)) |>
...
prod <- read_parquet(file.path(g_paths$dddata,"t_sf_aerel_gr34_pool_r.parquet")) |>
  filter(stat != "0") |>
  arrange(DOSGRP, variable, variable_level, stat_name)
