####------------------------------------------------------------------------####
# Program Name: qc_t_sf_dth_lot_pool_r.R
# Domino Environment: Multilingual Frozen R4.5.1_2025-09-09_v1 SASViya4
# Developer: Shilpa Kolli / sk575206
# Purpose: Perform Risk-Based Quality Control (RBQC) checks on Summary of Death by Line of Therapy
...
ard1 <- adbs |>
  filter(dthfl == "Y" ) |>
  mutate(dosgrp = factor(dosgrp, levels = dosgrp_levels), lotcat = factor(lotcat, levels = lot_levels)) |>
  ard_categorical(
    by = c(dosgrp, lotcat),
    variables = dthfl,
    denominator = adbs,
  ...
  mutate(variable = "Patient Deaths on Study, n(%)", variable_level = "Patient Deaths on Study, n(%)") |> arrange(group2_level) |>
  unlist_ard_columns()
...
ard2 <- adbs |>
  ...
  mutate(variable = "Primary Cause of Death") |> arrange(group2_level) |>
  unlist_ard_columns()
...
ard3 <- adbs |>
  ...
  mutate(variable = "Patient Deaths within 30 Days of Last IDRX-42 Dose, n(%)", variable_level = "Patient Deaths within 30 Days of Last IDRX-42 Dose, n(%)") |> arrange(group2_level) |>
  unlist_ard_columns()
...
ard4 <- adbs |>
  ...
  mutate(variable = "Primary Cause of Death within 30 Days") |> arrange(group2_level) |>
  unlist_ard_columns()
...
ard5 <- adbs |>
  ...
  mutate(variable = "Patient Deaths after 30 Days of Last IDRX-42 Dose, n(%)", variable_level = "Patient Deaths after 30 Days of Last IDRX-42 Dose, n(%)") |> arrange(group2_level) |>
  unlist_ard_columns()
...
ard6 <- adbs |>
  ...
  mutate(variable = "Primary Cause of Death after 30 Days") |> arrange(group2_level) |>
  unlist_ard_columns()
...
val_df <- bind_rows(final |> unlist_ard_columns(), bign |> unlist_ard_columns()) |>
  arrange(lotcat, arm, label)

prod_df <- read_parquet(file.path(g_paths$dddata, "t_sf_dth_lot_pool_r.parquet")) |> 
