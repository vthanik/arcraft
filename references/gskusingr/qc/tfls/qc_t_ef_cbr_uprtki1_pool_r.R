####------------------------------------------------------------------------####
# Program Name: qc_t_ef_cbr_uprtki1_pool_r
...
# Purpose: QC t_ef_cbr_uprtki1_pool_r
# Input: ADSL ADEFF
# Output: [N/A]
...
t_ef_cbr_uprtki1_pool_r <- read_parquet(file.path(
  g_paths$dddata,
  "t_ef_cbr_uprtki1_pool_r.parquet"
))

#
qc_t_ef_cbr_uprtki1_pool_r <- ard_tbl1 |>
...
t_ef_cbr_uprtki1_pool_r <- t_ef_cbr_uprtki1_pool_r |>
  arrange(column, stat_name, LABEL, GROUP) |>
  select(-ord_2)

# Compare PRODUCTION vs QC datasets ----
diffdf::diffdf(
  base = t_ef_cbr_uprtki1_pool_r,
  compare = qc_t_ef_cbr_uprtki1_pool_r
)