####------------------------------------------------------------------------####
# Program Name: qc_t_sf_aesocpt_rel_pool_r.R
...
prod <- read_parquet(file.path(g_paths$dddata,"t_sf_aesocpt_rel_pool_r.parquet")) |>
  filter(stat != "0") |>
  arrange(DOSGRP, variable, variable_level, stat_name)
...
qc_result <- diffdf(base = prod ,
                    ...
                    file = file.path(g_paths$qc_output, "qc_t_sf_aesocpt_rel_pool_r.lst")
)
