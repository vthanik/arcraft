####------------------------------------------------------------------------####
# Program Name: qc_t_sf_aeserrel_pool_r.R
# Domino Environment: Multilingual Frozen R4.5.1_2025-09-09_v1 SASViya4
# Developer: sk575206 / Shilpa Kolli
# Purpose: Summary of IDRX-42-related Treatment-Emergent Serious Adverse Events by System Organ Class 
#          and Preferred Term (Phase 1 & 1b pooled)
#
# Input:
#   - adam [adbs, adae]
# Output:
#   - qc_t_sf_aeserrel_pool_r.lst
#
...
# --- Load Libraries ---
libs <- c("dplyr", "forcats", "stringr", "purrr", "cards", "tfrmt", "gt", "admiral","arrow","diffdf")
pkg_attach(libs)
...
prod <- read_parquet(file.path(g_paths$dddata,"t_sf_aeserrel_pool_r.parquet")) |>
  filter(stat != "0") |>
  arrange(DOSGRP, variable, variable_level, stat_name)
...
diff_results <- diffdf(
  prod,
  qc,
  file = "/mnt/data/61834_300382_INTERNAL_05_ANALYSIS/misc/qc_t_sf_aeserrel_pool_r.lst"
)
print(diff_results)
