####------------------------------------------------------------------------####
# Program Name: t_sf_aewithdraw_pool_r.R
...
#   - ADaM: ADBS, ADAE
# Output:
#   - DD: t_sf_aewithdraw_pool_rparquet
#   - File: ../prod/output/t_sf_aewithdraw_pool_r.RTF
#
...
arrow::write_parquet(
  final_ard,
  file.path(g_paths$dddata, "t_sf_aewithdraw_pool_r.parquet")
)
...
rtf_obj <- gt |>
  ...
  rtf_title(
    ...
    " Preferred Term (Phase 1 & 1b pooled)"
  ) |>
  rtf_header(
  ...
  build_rtf(
    file = file.path(g_paths$output, "t_sf_aewithdraw_pool_r.rtf")
  )
