####------------------------------------------------------------------------####
# Program Name: t_sf_aerel_gr34_pool_r.R
# Domino Environment: Multilingual Frozen R4.5.1_2025-09-09_v1 SASViya4
# Developer: Shilpa Kolli / sk575206
# Purpose: Table 14.3.2.4c  -  Summary of IDRX-42-related Treatment-Emergent Adverse Events 
# with CTCAE Grade 3 or 4 by System Organ Class and Preferred Term (Phase 1 & 1b pooled)
#
# Input:
#   - ADaM: ADBS, ADAE
# Output:
#   - DD: t_sf_aerel_gr34_pool_r.parquet
#   - File: ../prod/output/t_sf_aerel_gr34_pool_r.RTF
#
...
adae_raw <- read_data_catalog(adamdata$adae) |>
  filter(SAFFL == "Y" & ATOXGR %in% c("Grade 3", "Grade 4") & AREL == "Related" & TRTEMFL == "Y")
...
arrow::write_parquet(
  final_ard,
  file.path(g_paths$dddata, "t_sf_aerel_gr34_pool_r.parquet")
)
...
rtf_obj <- gt |>
  ...
  rtf_title(
    "Table 14.3.2.4c",
    "Summary of IDRX-42-related Treatment-Emergent Adverse Events with CTCAE Grade 3 or 4 by System Organ Class",
  ...
  build_rtf(
    file = file.path(g_paths$output, "t_sf_aerel_gr34_pool_r.rtf")
  )