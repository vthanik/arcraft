####------------------------------------------------------------------------####
# Program Name: t_ef_orr_muttstt2_pool_r.R
# Domino Environment: Multilingual Frozen R4.5.1_2025-09-09_v1 SASViya4
# Developer: Naga Laxmi Chinthala / nc450888
# Purpose: Summary of Best Overall Response and Objective Response Rate by Mutation Status in 2nd-line Therapy (Phase 1 & 1b pooled)
#
# Input:
#   - adam [adbs, adeff]
# Output:
#   - dd [t_ef_orr_muttstt2_pool_r]
#   - File: ../prod/output/t_ef_orr_muttstt2_pool_r.RTF
#
...
adbs <- read_data_catalog(adamdata$adbs) %>%
  filter(EFFL == "Y" & !is.na(TRT01A), LOTCAT == "2nd-line") %>%
...
adbsx <- adbs %>%
  ...
  bind_rows(adbs) %>%
  mutate(
    ARM = factor(ARM, levels = trt_levels_total, labels = trt_levels_total)
    )
...
all_exons <- c("Exon 9", "Exon 11", "Exon 13", "Exon 14", "Exon 17")
...
ard_tbl <- ard %>%
  rename_ard_columns(columns = c("group2")) %>%
  unlist_ard_columns() %>%
  mutate(
    label = map_chr(
      variable_level,
...
arrow::write_parquet(ard_tbl, file.path(g_paths$dddata, "t_ef_orr_muttstt2_pool_r.parquet"))
...
rtf_obj <- gt |> 
  ...
  rtf_title(
    "Table 14.2.1.1g",
    "Summary of Best Overall Response and Objective Response Rate by Mutation Status in 2nd-line Therapy",
    "(Phase 1 & 1b pooled)"
  ) |>
  rtf_header(
  ...
  build_rtf(file = file.path(g_paths$output, "t_ef_orr_muttstt2_pool_r.rtf"))