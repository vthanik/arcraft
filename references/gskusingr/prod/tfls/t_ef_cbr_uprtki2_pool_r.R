####------------------------------------------------------------------------####
# Program Name: t_ef_cbr_uprtki2_pool_r.R
...
# Purpose: To create Table 14.2.6.2c  - Summary of Clinical Benefit Rate in Subjects with Prior Ripretinib
...
# left join adbs to boru_orr, create CBRFL and Y/N flag
bor_resp <- adbs1 %>%
  select(usubjid) %>%
  left_join(boru_orr %>% mutate(.drv = TRUE), by = "usubjid") %>%
  mutate(paramcd = "CBRFL", avalc = if_else(!is.na(.drv) & .drv, "Y", "N")) %>%
  select(-.drv)
...
rtf_dt <-
  ...
  rtf_title(
    "Table 14.2.6.2c",
    "Summary of Clinical Benefit Rate in Subjects with Prior Ripretinib (Phase 1 & 1b pooled)",
...
rtf_dt |>
  build_rtf(file = file.path(g_paths$output, "t_ef_cbr_uprtki2_pool_r.rtf"))


# Write out ARD to parquet file
write_parquet(df_borci, file.path(g_paths$dddata, "t_ef_cbr_uprtki2_pool_r.parquet"))

#Reading parquet file
t_ef_cbr_uprtki2_pool_r <- read_parquet(file.path(g_paths$dddata, "t_ef_cbr_uprtki2_pool_r.parquet"))