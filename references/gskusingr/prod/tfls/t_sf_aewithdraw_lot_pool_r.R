####------------------------------------------------------------------------####
# Program Name: t_sf_aewithdraw_lot_pool_r.R
# Domino Environment: Multilingual Frozen R4.5.1_2025-09-09_v1 SASViya4
# Developer: Shilpa Kolli / sk575206
# Purpose: Table 14.3.2.9c  -   Summary of Treatment-Emergent Adverse Events Leading to IDRX-42 Withdrawn
# by System Organ Class and Preferred Term (Phase 1 & 1b pooled)-  Safety Analysis Set
#
# Input:
#   - ADaM: ADBS, ADAE
# Output:
#   - DD: t_sf_aewithdraw_lot_pool_rparquet
#   - File: ../prod/output/t_sf_aewithdraw_lot_pool_r.RTF
#
...
adae_by_trt <- adae_raw |>
  ...
  inner_join(select(adbs_raw, USUBJID, LOTCAT, DOSGRP), by = "USUBJID")
...
final_ard_ <- pivot_longer(
  data = final_ard1,
  cols = starts_with(c("C", "T")),
  names_to = "DOSGRP",
  values_to = "stat"
)

final_ard <- bind_rows(
  final_ard |> filter(LOTCAT == "1st-line" & variable == "Patients with Any TEAE"),
  final_ard_
)

arrow::write_parquet(
  final_ard,
  file.path(g_paths$dddata, "t_sf_aewithdraw_lot_pool_r.parquet")
)
...
rtf_obj <- gt |>
  ...
  rtf_title(
    "Table 14.3.2.9e",
    "Summary of Treatment-Emergent Adverse Events Leading to IDRX-42 Withdrawn by System Organ Class and",
    " Preferred Term by Line of Therapy (Phase 1 & 1b pooled)"
  ) |>
  rtf_header(
  ...
  build_rtf(
    file = file.path(g_paths$output, "t_sf_aewithdraw_lot_pool_r.rtf")
  )
