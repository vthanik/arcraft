####------------------------------------------------------------------------####
# Program Name: f_ef_kmplot_2l_pool_r.R
...
# Output:
#   - File: ../prod/output/f_ef_kmplot_2l_pool_r.rtf
#
...
adbs_raw <- read_data_catalog(adamdata$adbs) |>
  filter(EFFL == "Y", !is.na(DOSGRP), LOTCAT == "2nd-line")
...
rtf_obj <- survplot |>
  ...
  rtf_title(
    "Figure 14.2.2e",
    "Kaplan-Meier Plot for Progression-free Survival in 2nd Line of Therapy (Phase 1 & 1b pooled)"
  ) |>
  rtf_header(
  ...
  build_rtf(
    file = file.path(g_paths$output, "f_ef_kmplot_2l_pool_r.rtf")
  )