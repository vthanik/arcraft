...
arrow::write_parquet(
  final_ard,
  file.path(g_paths$dddata, "t_sf_aesocpt_pool_r.parquet")
)
...
rtf_obj <- gt |>
  ...
  build_rtf(
    file = file.path(g_paths$output, "t_sf_aesocpt_pool_r.rtf")
  )
