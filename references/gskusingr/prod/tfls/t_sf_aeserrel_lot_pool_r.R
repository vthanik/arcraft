...
####------------------------------------------------------------------------####
# Program Name: t_sf_aeserrel_lot_pool_r.R
# Domino Environment: Multilingual Frozen R4.5.1_2025-09-09_v1 SASViya4
# Developer: Pragya Tripathi / ps209009
# Purpose: Table 14.3.2.2e Summary of IDRX-42-related Serious Treatment-Emergent 
...
#   - adam [adbs, adae]
# Output:
#   - dd [t_sf_aeserrel_lot_pool_r]
#   - File: ../prod/output/t_sf_aeserrel_lot_pool_r.RTF
#
...
adae_raw <- read_data_catalog(adamdata$adae) |>
  filter(SAFFL == "Y" & AREL == "Related" & AESER=="Y" & TRTEMFL == "Y")
...
main_stats_ard <- reshaped_ard |>
  ...
  mutate(
    ...
    ord=ifelse(variable=="Patients with Any TEAE",1,2)
  ) |>
  arrange(LOTCAT, DOSGRP, desc(soc_count), desc(all_count),ord, variable) |>
...
arrow::write_parquet(
  final_ard,
  file.path(g_paths$dddata, "t_sf_aeserrel_lot_pool_r.parquet")
)
...
rtf_obj <- gt |>
  ...
  rtf_title(
    "Table 14.3.3.2e",
    "Summary of IDRX-42-related Treatment-Emergent Serious Adverse Events by System Organ Class and Preferred Term",
  ...
  build_rtf(
    file = file.path(g_paths$output, "t_sf_aeserrel_lot_pool_r.rtf")
  )
