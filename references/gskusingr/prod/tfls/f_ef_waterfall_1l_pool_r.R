####------------------------------------------------------------------------####
# Program Name: f_ef_waterfall_1l_pool_r.R
# Domino Environment: Multilingual Frozen R4.5.1_2025-09-09_v1 SASViya4
# Developer: Naga Laxmi Chinthala / nc450888
# Purpose: Waterfall Plot for Best Percentage Change of Target Lesion from Baseline
# in 1st-line Therapy (Phase 1 & 1b pooled)
#
# Input:
#   - ADAM: ADBS, ADTR, ADEFF
# Output:
#   - File: ../prod/output/f_ef_waterfall_1l_pool_r.rtf
#
...
adbs_safety <- read_data_catalog(adamdata$adbs) |>
  filter(EFFL == "Y", !is.na(DOSGRP), LOTCAT == "1st-line")
...
write_parquet(waterfall_analysis_data, file.path(g_paths$dddata, "f_ef_waterfall_1l_pool_r.parquet"))

#Reading parquet file
f_ef_waterfall_1l_pool_r <- read_parquet(file.path(g_paths$dddata, "f_ef_waterfall_1l_pool_r.parquet"))

# --- 3. Generate Plots by DOSGRP (All LOTs) ---

colors_by_trt <- adbs_safety |> distinct(DOSGRPN, DOSGRP) |> pull(DOSGRPN)

plots_by_treatment_group <- fig_waterfall(
  data = waterfall_analysis_data,
  subj_col = SUBJID,
  pchg_col = PCHG,
  group_col = DOSGRP,
...
rtf_obj <- plots_by_treatment_group |>
  ...
  rtf_title(
    ...
    "in 1st Line of Therapy (Phase 1 & 1b pooled)"
  ) |>
  rtf_header(
  ...
  build_rtf(
    file = file.path(g_paths$output, "f_ef_waterfall_1l_pool_r.rtf")
  )