####------------------------------------------------------------------------####
# Program Name: t_ef_orr_uprtki3_pool_r.R
...
# Purpose: To create Table 14.2.1.1.2.3c  - Summary of Objective Response Rates in Subjects with Prior Ripretinib,
...
# merge with adbs to create ORRFL Y/N
bor_resp <- adbs1 %>%
...
df_borci1 <- df_borci_ %>%
  ...
  filter(
    !((N == 0) & stat_name %in% c("conf.low", "conf.high"))) %>% #removing zero rows
  select(-N)
...
rtf_dt |>
  build_rtf(file = file.path(g_paths$output, "t_ef_orr_uprtki3_pool_r.rtf"))

# Write out ARD to parquet file
write_parquet(df_borci,file.path(g_paths$dddata, "t_ef_orr_uprtki3_pool_r.parquet"))

#Reading parquet file
t_ef_orr_uprtki3_pool_r <- read_parquet(file.path(g_paths$dddata,"t_ef_orr_uprtki3_pool_r.parquet"))