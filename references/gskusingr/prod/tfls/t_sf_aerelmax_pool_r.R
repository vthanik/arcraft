####------------------------------------------------------------------------####
# Program Name: t_sf_aerelmax_pool_r.R
# Domino Environment: Multilingual Frozen R4.5.1_2025-09-09_v1 SASViya4
# Developer: Shilpa Kolli / sk575206
# Purpose: Table 14.3.2.8c Summary of IDRX-42-related Treatment-Emergent Adverse Events by System 
# Organ Class, Preferred Term and Maximum CTCAE Grade (Phase 1 & 1b pooled)
#
# Input:
#   - ADaM: [adbs, adae]
# Output:
#   - DD: t_sf_aerelmax_pool_r.parquet
#   - File: ../prod/output/t_sf_aerelmax_pool_r.RTF
#
...
df_N <- adbs_raw |>
  ...
  bind_rows(adbs_raw |> distinct(USUBJID) |> mutate(DOSGRP = "Total")) |>
  count(DOSGRP, name = "N")

# Create the 'bign' object for tfrmt column labels
bign <- df_N |>
  mutate(col_label = if_else(N > 0, paste0(DOSGRP, " (N=", N, ")"), DOSGRP))
...
calculate_max_grade_by <- function(.data, ...) {
  ...
  bind_rows(
    df_max_grade,
    df_max_grade |> distinct(USUBJID, DOSGRP, ...) |> mutate(ATOXGR = "Total")
  )
}
...
df_counts <- adae_hierarchical_n |>
  bind_rows(mutate(adae_hierarchical_n, DOSGRP = "Total")) |>
  group_by(DOSGRP, ATOXGR, variable, AESOC, AEDECOD, variable_level) |>
  summarise(n = n_distinct(USUBJID), .groups = "drop")

# 1. Create a scaffold of the column structure for EACH treatment group
df_scaffold <- expand_grid(
  DOSGRP = factor(trt_levels_total, levels = trt_levels_total),
...
hier_items <- df_counts |>
  distinct(DOSGRP, variable, AESOC, AEDECOD, variable_level)
...
pt_total_counts <- reshaped_ard |>
  filter(DOSGRP == "Total", ATOXGR == "Total", stat_name == "n", !is.na(AEDECOD)) |>
  select(AEDECOD, pt_count = stat)

soc_total_counts <- reshaped_ard |>
  filter(DOSGRP == "Total", ATOXGR == "Total", stat_name == "n", variable_level == "Any event") |>
  select(variable, soc_count = stat)

# Create final main statistics ARD with sorting applied.
main_stats_ard <- reshaped_ard |>
  left_join(pt_total_counts, by = "AEDECOD") |>
  ...
  arrange(desc(is_any_teae), desc(is_any_event), desc(soc_count), desc(pt_count), variable, variable_level, DOSGRP) |> 
  left_join(select(bign, DOSGRP, col_label), by="DOSGRP") |>
  select(DOSGRP, col_label, variable, variable_level, ATOXGR, stat_name, stat) |>
...
arrow::write_parquet(
  main_stats_ard,
  file.path(g_paths$dddata, "t_sf_aerelmax_pool_r.parquet")
)
...
rtf_obj <- gt |>
  rtf_table(
    page_group_label = paste("Treatment:", col_label),
    drop_columns = DOSGRP
  ) |>
  rtf_col_align(
    variable_level = "left",
  ...
  rtf_title(
    ...
    "Maximum CTCAE Grade (Phase 1 & 1b pooled)"
  ) |>
  rtf_header(
  ...
  build_rtf(
    file = file.path(g_paths$output, "t_sf_aerelmax_pool_r.rtf")
  )
