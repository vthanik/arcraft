####------------------------------------------------------------------------####
# Program Name: t_sf_aerelgr34_12l_pool_r.R
# Domino Environment: Multilingual Frozen R4.5.1_2025-09-09_v1 SASViya4
# Developer: Vignesh Thanikachalam / vt704670
# Purpose: Table 14.3.2.4f Summary of IDRX-42-related Treatment-Emergent Adverse
# Events with CTCAE Grade 3 or 4 by System Organ Class and Preferred Term 
# by 1st and 2nd Line of Therapy (Phase 1 & 1b pooled)
#
# Input:
#   - ADaM: ADBS, ADAE
# Output:
#   - DD: t_sf_aerelgr34_12l_pool_r.parquet
#   - File: ../prod/output/t_sf_aerelgr34_12l_pool_r.RTF
#
...
adbs_raw <- read_data_catalog(adamdata$adbs) |>
  filter(SAFFL == "Y", !is.na(DOSGRP), LOTCAT %in% c("1st-line", "2nd-line"))

# Load Adverse Event data for the Safety population.
adae_raw <- read_data_catalog(adamdata$adae) |>
  filter(SAFFL == "Y" & TRTEMFL == "Y" & AREL == "Related" & ATOXGR %in% c("Grade 3", "Grade 4"))

# Dynamically create ordered Treatment Group factor levels.
trt_levels <- read_data_catalog(adamdata$adbs) |>
  filter(!is.na(DOSGRP)) |> 
...
main_stats_ard <- reshaped_ard |>
  ...
  mutate(
    ...
    is_any_evnt = (variable_level == "Any event"),
  ...
  arrange(LOTCAT, DOSGRP, desc(is_any_teae), desc(is_any_evnt), desc(soc_count), desc(all_count), variable, variable_level) |>
...
arrow::write_parquet(
  final_ard,
  file.path(g_paths$dddata, "t_sf_aerelgr34_12l_pool_r.parquet")
)
...
rtf_obj <- gt |>
  ...
  rtf_options(page_line_size_landscape = 31) |> 
  rtf_title(
    "Table 14.3.2.2e",
    "Summary of IDRX-42-related Treatment-Emergent Adverse Events with CTCAE Grade 3 or 4",
    "by System Organ Class and Preferred Term by 1st and 2nd Line of Therapy (Phase 1 & 1b pooled)",
  ) |>
  rtf_header(
  ...
  build_rtf(
    file = file.path(g_paths$output, "t_sf_aerelgr34_12l_pool_r.rtf")
  )
