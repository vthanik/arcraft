####------------------------------------------------------------------------####
# Program Name: t_sf_aeser_lot_pool_r.R
# Domino Environment: Multilingual Frozen R4.5.1_2025-09-09_v1 SASViya4
# Developer: Pragya Tripathi / ps209009
# Purpose: Table 14.3.3.1e Summary of Treatment-Emergent Serious Adverse Events 
# by System Organ Class and Preferred Term by line of therapy subgroup (Phase 1 & 1b pooled)
#
# Input:
#   - adam [adbs, adae]
# Output:
#   - dd [t_sf_aeser_lot_pool_r]
#   - File: ../prod/output/t_sf_aeser_lot_pool_r.RTF
#
...
adae_raw <- read_data_catalog(adamdata$adae) |>
  filter(SAFFL == "Y" & AESER == "Y" & TRTEMFL == "Y")
...
pop <-
  ...
  mutate(DOSGRP = factor(DOSGRP, levels = trt_levels_total), LOTCAT = factor(LOTCAT, levels = lot_levels))
...
adae_by_trt <- adae_raw |>
  ...
  left_join(select(adbs_raw, USUBJID, DOSGRP, LOTCAT), by = "USUBJID")
...
reshaped_ard_pre <- processed_ard |>
...
filterout_soc_pt <- reshaped_ard_pre|> 
  filter(DOSGRP=="Total",stat==0) |> 
  select(LOTCAT,variable,variable_level,context,stat_name,stat_label) |> 
  mutate(exc_flg="Y")

reshaped_ard_flg <- reshaped_ard_pre |> 
  left_join(filterout_soc_pt,by=c("LOTCAT","variable","variable_level",
                                  "context","stat_name","stat_label")) 

reshaped_ard <- reshaped_ard_flg |>
  filter(is.na(exc_flg))
...
main_stats_ard <- reshaped_ard |>
  left_join(total_counts, by = c("LOTCAT", "variable", "variable_level")) |>
  ...
  arrange(LOTCAT, DOSGRP, desc(soc_count), desc(all_count), variable) |>
...
bign <- pop %>%
  ...
  arrange(LOTCAT,trtn) %>% select(LOTCAT, DOSGRP, stat_name, stat)
...
arrow::write_parquet(
  final_ard,
  file.path(g_paths$dddata, "t_sf_aeser_lot_pool_r.parquet")
)
...
rtf_obj <- gt |>
  ...
  rtf_title(
    "Table 14.3.3.1e",
    "Summary of Treatment-Emergent Serious Adverse Events by System Organ Class and Preferred Term",
  ...
  build_rtf(
    file = file.path(g_paths$output, "t_sf_aeser_lot_pool_r.rtf")
  )
