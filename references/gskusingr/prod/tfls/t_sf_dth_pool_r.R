...
ard_tbl <-
  ...
  select(ARM, variable, stat_name, stat, label, ord) %>%
  mutate(
    ord2 = case_when(
      # For CMLINE
  ...
  arrange( ord, ord2, variable, label) %>%
  select(-c(ord, ord2))
...
arrow::write_parquet(ard_tbl, file.path(g_paths$dddata, "t_sf_dth_pool_r.parquet"))
...
rtf_obj <- THL |> 
  ...
  rtf_title(
    "Table 14.3.3.6c",
    "Summary of Death (Phase 1 & 1b pooled)"
  ) |>
  rtf_header(
  ...
  build_rtf(file = file.path(g_paths$output, "t_sf_dth_pool_r.rtf"))