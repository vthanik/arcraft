####------------------------------------------------------------------------####
# Program Name: f_ef_waterfall_2l_pool_r.R
# Domino Environment: Multilingual Frozen R4.5.1_2025-09-09_v1 SASViya4
# Developer: Vignesh Thanikachalam / vt704670
# Purpose: Waterfall Plot for Best Percentage Change of Target Lesion
# in 2nd-line Therapy
#
# Input:
#   - ADAM: ADBS, ADTR, ADEFF
# Output:
#   - File: ../prod/output/f_ef_waterfall_2l_pool_r.rtf
#
...
adbs_safety <- read_data_catalog(adamdata$adbs) |>
  filter(SAFFL == "Y", !is.na(DOSGRP), LOTCAT == "2nd-line")
...
rtf_obj <- plots_by_treatment_group |>
  ...
  rtf_title(
    ...
    "in 2nd Line of Therapy (Phase 1 & 1b pooled)"
  ) |>
  rtf_header(
  ...
  build_rtf(
    file = file.path(g_paths$output, "f_ef_waterfall_2l_pool_r.rtf")
  )
