####------------------------------------------------------------------------####
# Program Name: t_sf_aept_5pct_pool.R
...
# Purpose: Table 14.3.2.12c-Summary of Treatment-Emergent Adverse Events >=5% of 
# Subjects by Preferred Term, (Phase 1 & 1b pooled)
# Input: ADBS, ADAE
# Output:
#
...
# --- 3. ARD using ard_stack_hierarchical (PT only) ---
ard <- adae |>
...
# --- 5. Apply ≥5% filter (any active group) ---
pt_keep <- processed_ard |>
  ...
  summarise(max_pct = max(stat, na.rm = TRUE), .groups = "drop") |>
  filter(max_pct >= 5) |>
  pull(variable_level)
...
# --- 6. Replace Overall Row and Prepare Final ARD ---
processed_ard <- processed_ard |>
  filter(variable != "DOSGRP") |>
  mutate(
    variable_level = as.character(variable_level), # convert all to character
...
write_parquet(final_ard, file.path(g_paths$dddata, "t_sf_aept_5pct_pool_r.parquet"))
...
rtf_obj <- gt |>
  ...
  rtf_title(
    "Table 14.3.2.12c",
    "Summary of Treatment-Emergent Adverse Events >=5% of Subjects by Preferred Term",
    "(Phase 1 & 1b pooled)"
  ) |>
  rtf_header(
  ...
  build_rtf(
    file = file.path(g_paths$output, "t_sf_aept_5pct_pool_r.rtf")
  )
