...
| Date:               18May2007
|
| Macro Purpose:      A macro to produce a summary table. The number and percentage 
|                     of subjects who completed the study as planned as well subjects 
|                     who stopped investigational product prematurely will be 
|                     produced (SD1)
|
...
|                     tu_freq or tu_sumstatsinrows and             rdata.ipdisc,group
|                     appropriate parameters for              the  ByVarsNumer=&g_trt
|                     macro to create an output dataset. The       cd
|                     percentage sign shall not be used as a       sdstopp,DSETOUT=se
|                     prefix to the macro name, and the            gment1_out(rename=
|                     parameters shall not be surrounded by        (sdstopp=tt_code1)
|                     brackets, i.e. the following style shall     ),ROWLABELVARNAME=
|                     be used:  segment1=MacroName                 tt_grplabel='Compl
|                     parm1=value1, parm2=value2                   etion Status')
...
|                                                                  sdsubr)
|
| SEGMENT4-SEGMENT20  Same as SEGMENT1                             (Blank)
|
...
|                     and variable names                           sdrscd tt_code1
...
|                     that are given the PROC REPORT define        sdrscd tt_code1
...
|                     display.
|
| NOWIDOWVAR          Variable whose values must be kept together  sdrscd
|                     on a page
...
|                     COLUMNS
|
| POSTSUBSET          SAS expression to be applied to data         %nrstr(if
|                     immediately prior to creation of the         tt_segorder ge 2
|                     permanent presentation dataset. Used for     then do;
|                     subsetting records required for computation  tt_segorder=2;
|                     but not for display.                         tt_grplabel='Prima
|                     Valid values: Blank or a complete,           ry[1]/subreason[2]
|                     syntactically valid SAS where or if          for
|                     statement for use in a data step             discontinuation';
|                                                                  end; else
|                                                                  tt_decode1=put(tt_
|                                                                  code1, $sdstop.))
|
...
|
| XMLDEFAULTS         Specifies a XML file which defined           &g_refdata/tr_sd1a
|                     parameter values of %tu_freq or              _defaults.xml
...
%macro td_sd1a (
   SEGMENT1            =%str(dsetinnumer=ardata.ipdisc,groupByVarsNumer=&g_trtcd sdstopp,DSETOUT=segment1_out(rename=(sdstopp=tt_code1)),ROWLABELVARNAME=tt_grplabel='Completion Status'), /* Same as SEGMENT4 */
   SEGMENT2            =%str(dsetinnumer=ardata.ipdisc(where=(sdstopp eq 'Y')), groupByVarsNumer=&g_trtcd sdrscd, DSETOUT=segment2_out, POSTSUBSET=if 1 then tt_decode1=sdrs), /* Same as SEGMENT4 */
   SEGMENT3            =%str(dsetinnumer=ardata.ipdisc(where=(sdstopp eq 'Y' and not missing(sdsubrcd))),DSETOUT=segment3_out(rename=(sdsubrcd=tt_code1)),POSTSUBSET=if 1 then tt_decode1='  '||sdsubr), /* Same as SEGMENT4 */
   ...
   COLUMNS             =tt_segorder tt_grplabel sdrscd tt_code1 tt_decode1 tt_ac:, /* Column parameter */
   ...
   DDDATASETLABEL      =DD dataset for new SD1 table, /* Label to be applied to the DD dataset */
   ...
   NOPRINTVARS         =tt_segorder sdrscd tt_code1, /* No print vars (usually used to order the display) */
   NOWIDOWVAR          =sdrscd,            /* Variable whose values must be kept together on a page */
   ORDERDATA           =,                  /* ORDER=DATA variables */
   ORDERFORMATTED      =,                  /* ORDER=FORMATTED variables */
   ORDERFREQ           =,                  /* ORDER=FREQ variables */
   ORDERVARS           =tt_segorder tt_grplabel sdrscd tt_code1 tt_decode1, /* Order variables */
   PAGEVARS            =,                  /* Break after <var> / page; */
   POSTSUBSET          =%nrstr(if tt_segorder ge 2 then do; tt_segorder=2; tt_grplabel='Primary[1]/subreason[2] for discontinuation'; end; else tt_decode1=put(tt_code1, $sdstop.)), /* SAS expression to be applied to data before call %tu_list */
   ...
   XMLDEFAULTS         =&g_refdata/tr_sd1a_defaults.xml /* Location and name of XML defaults file */
   );
   ...
   %if &l_flag %then
   %do;
      libname _tdsd1a xml "&xmldefaults";
      proc sql noprint;
         select count(memname) into :l_flag
         from dictionary.tables
         where libname eq '_TDSD1A' and memname eq 'SD1A'
         ;
      quit; 
      %if not &l_flag %then
      %do;      
         libname _tdsd1a clear;
      %end;       
   %end;
