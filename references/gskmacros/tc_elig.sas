...
| Date:            29-Jun-2004
|
| Macro Purpose:   Elig wrapper macro
|
...
| -----------------  -------------------------------------  -------  ----------
| DSETIN             Specifies the ELIG-format SI dataset   REQ      DMDATA.ELIG
|                    which needs to be transformed into an 
|                    ELIG-format A&R dataset.
|                    Valid values: valid dataset name 
|
| DSETOUT            Specifies the name of the output       REQ      ARDATA.ELIG
...
*******************************************************************************/
%macro tc_elig (
     dsetin            = DMDATA.ELIG,  /* Input dataset name */
 ...
 %local prefix;
 %let prefix = _tc_elig;   /* Root name for temporary work datasets */
 ...
 /*
 / Only keep records with non-missing Criteria type codes (IECRTYCD) 
 / and non-missing Criteria codes (IECRTNUM).                   
 /----------------------------------------------------------------------------*/

 data &prefix._ds&i;
      set %unquote(&dsetin);
      if (iecrtycd ne ' ') and (iecrtnum ne ' ');
 run;
