...
| Date:            28-Jun-2004
|
| Macro Purpose:   Stopdrug wrapper macro
|
...
| -----------------  -------------------------------------  -------  ----------
| DSETIN             Specifies the STOPDRUG-format SI       REQ      DMDATA.STOPDRUG
|                    dataset which needs to be transformed 
|                    into a STOPDRUG-format A&R dataset.
|                    Valid values: valid dataset name 
|
| DSETOUT            Specifies the name of the output       REQ      ARDATA.STOPDRUG
|                    dataset to be created.
|                    Valid values: valid dataset name 
|
| EXPOSUREDSET       Specifies the name of the SI-format    OPT      DMDATA.EXPOSURE
|                    EXPOSURE dataset to use to derive 
|                    SDENDT, SDENTM. 
|                    Valid values: valid dataset name 
|
...
|                    derivations for this domain code (SD)?
|                    Valid values: Y, N
|
...
|                    Valid values: Y, N
|
| REFDAT             Specify a reference date variable name OPT      SDENDT
|                    to pass to %tu_recalcvisit to  
|                    calculate the visit. Will be checked  
|                    in %tu_recalcvisit
|
| REFTIM             Specify a reference time variable name OPT      SDENTM
|                    to pass to %tu_recalcvisit to  
|                    calculate the visit. Will be checked  
|                    in %tu_recalcvisit
|
...
| Date of Modification:     21-Mar-2005
| New version/draft number: 1/2
| Modification ID:          YW001
| Reason For Modification:  Moved the call to %tu_derive before call to
|                           %tu_recalcvisit, because sdendt and sdentm 
|                           are derived from %tu_derive.  Moved check for 
|                           REFTIM parameter for similar reasons.
*******************************************************************************
| Modified By:              Yongwei Wang
| Date of Modification:     04-May-2005
| New version/draft number: 1/3
| Modification ID:          YW002
| Reason For Modification:  Modified one RTERROR message and one RTWARNING
|                           message.
*******************************************************************************
| Modified By:              Yongwei Wang
...
%macro tc_stopdrug (
     dsetin            = DMDATA.STOPDRUG,  /* STOPDRUG dataset name */
     dsetout           = ARDATA.STOPDRUG,  /* Output dataset name */
     ...
     refdat            = sdendt,     /* Reference data variable name for recalculating visit and calculating treatment period */
     reftim            = sdentm,     /* Reference data variable name for recalculating visit and calculating treatment period */
 ...
 %local prefix;
 %let prefix = _tc_stopdrug;   /* Root name for temporary work datasets */
 ...
 %do;
    %tu_derive (
         ...
         domaincode        = sd,                     /* Domain Code - type of dataset */
 ...
 %do;
    ...
    %do;
       %put %str(RTW)ARNING: Variable REFTIM(=&reftim) does not exist in DSETIN(=&dsetin). It will not be used to recalculate visit.;
       %let reftim=;
    %end;
 %end; 
