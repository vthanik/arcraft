...
| Date:            19-Jun-2005
|
| Macro Purpose:   To create NONMEM files for parallel-group studies.
|
...
|                   information. 
|
| DVILEAVEBY        Specifies the variable(s) by which  &g_subjid visitnum (Req)
...
| Example:
|
| %tc_nonmemp(dsetinexp = ardata.exposure 
|            ,expcmt = 0
...
|            ,colorder = subjid visitnum date tim2
|            );
|
|******************************************************************************* 
| Change Log 
...
|                           Add &G_CENTID to default for DVILEAVEBY.
...
| Modification ID:          AR3
| Reason For Modification:  Add SORTBY to %tu_nmcv call.
|
| Modified By:              Andrew Ratcliffe, RTSL
...
| Modification ID:          
| Reason For Modification:  Set COLORDER default to &g_subjid &outdate &outtime.
...
%macro tc_nonmemp(
                 ...
                 ,dvileaveby    = &g_subjid visitnum /* Variable(s) by which dependant variable(s) shall be interleaved */
