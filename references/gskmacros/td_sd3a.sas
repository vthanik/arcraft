...
| Date:               01Jun2007
|
| Macro purpose:      display macro to generate new IDSL SD3 listing
|
...
|                     PROC REPORT COLUMN statement syntax, but     sdacttrt actdt 
|                     not including report_item=alias syntax       st_all_spd      
|                                                                  sdrscd sdrs      
|                                                                  sdsubrcd sdsubr  
|                                                                                   
|
...
|                     DD macro.  This parameter specifies column   sdacttrt 12
|                     widths for all variables not listed in the   actdt 10 
|                     WIDTHS parameter.                            st_all_spd 7 sdrs 22
|                     Valid values: values of column names and     sdsubr 22
...
|                     option.                                      st_all_ptp
|                     Valid values: one or more variable names     sdacttrt sdrs
|                     from DSETIN that are also defined with       sdsubr
|                     COLUMNS                                      st_all_spd
...
|
| NOPRINTVARS         Variables listed in the COLUMN parameter     &g_trtseqcd sdrscd
|                     that are given the PROC REPORT define        sdsubrcd
...
|                     variables specified in COLUMNS               sdrscd
|                                                                  sdsubrcd
|
...
|                                                                  sepc=/)
|
| STACKVAR3           (Same as STACKVAR1)                          %str(varsin=actdy
|                                                                  perdy,
|                                                                  varout=st_all_spd,
|                                                                  sepc=/)
|
...
%macro td_sd3a(
   ...
   COLUMNS             =&g_centid &g_subjid st_all_cs &g_trtseqcd st_all_ptp sdacttrt actdt st_all_spd sdrscd sdrs sdsubrcd sdsubr, /* Column parameter */
   ...
   DDDATASETLABEL      =DD dataset for new SD3 listing, /* Label to be applied to the DD dataset */
   DEFAULTWIDTHS       =st_all_cs 8 st_all_ptp 9 sdacttrt 12 actdt 10 st_all_spd 7 sdrs 22 sdsubr 22, /* List of default column widths */
   DESCENDING          =,                  /* Descending ORDERVARS */
   DSETIN              =ardata.ipdisc,         /* Input dataset */
   FLOWVARS            =st_all_cs st_all_ptp sdacttrt sdrs sdsubr st_all_spd, /* Variables with flow option */
   ...
   NOPRINTVARS         =&g_centid &g_subjid &g_trtseqcd sdrscd sdsubrcd, /* No print vars (usually used to order the display) */
   ...
   ORDERVARS           =&g_centid &g_subjid st_all_cs &g_trtseqcd st_all_ptp sdacttrt actdt st_all_spd sdrscd sdrs sdsubrcd, /* Order variables */
   ...
   STACKVAR3           =%str(varsin=actdy perdy, varout=st_all_spd, sepc=/), /* (Same as STACKVAR1) */
