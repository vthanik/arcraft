...
| Date:               24Aug2007
|
| Macro Purpose:      A macro to create will produce a summary table to summarize
|                     the recorded outcomes for the liver event assessment.
|
...
|                     brackets, i.e. the following style shall     s reporting a
|                     be used:  segment1=MacroName                 significant#
|                     parm1=value1, parm2=value2                   liver chemistry
|                     Valid values: A complete call to tu_freq or  result')
|                     ru_sumstatsinrows
|
...
|                                                                  $yesnoyn.)
|
| SEGMENT11-SEGMENT2  Same as SEGMENT1                             (Blank)
| 0
|
...
|                     format on the DSETIN.                        tt_grplabel
|                     Valid values: values of column names and
...
|                     supplied to this parameter then all          tt_decode1
...
|                     immediately prior to creation of the         then
|                     permanent presentation dataset. Used for     tt_code1='A'; if
|                     subsetting records required for computation  tt_segorder gt 2
|                     but not for display.                         then tt_decode1='
...
|                     parameter values of %tu_freq or              r1_defaults.xml
...
%macro td_liver1 (
   SEGMENT1            =%str(dsetindenom=&g_popdata,groupByVarsNumer=&g_trtcd,POSTSUBSET=tt_segorder=1;tt_grplabel='';tt_code1='1';tt_decode1='Subjects reporting a significant# liver chemistry result'), /* Same as SEGMENT12 */
   ...
   SEGMENT3            =%str(dsetinnumer=ardata.rucam(where=(rutestcd eq 'LVCHMEVT')),DSETOUT=segment3_out(rename=(ruorrscd=tt_code1 ruorres=tt_decode1)),POSTSUBSET=tt_grplabel=trim(rutest)||'*',PSFORMAT=&g_trtcd &g_trtfmt ruorrscd $lvchmes.),
   SEGMENT4            =%str(dsetinnumer=ardata.rucam(where=(rutestcd eq 'AGE55'   )),DSETOUT=segment4_out(rename=(ruorrscd=tt_code1 ruorres=tt_decode1 rutest=tt_grplabel)),PSFORMAT=&g_trtcd &g_trtfmt ruorrscd $yesnoyn.), /* Same as SEGMENT12 */
   SEGMENT5            =%str(dsetinnumer=ardata.rucam(where=(rutestcd eq 'PGYN'    )),DSETOUT=segment5_out(rename=(ruorrscd=tt_code1 ruorres=tt_decode1 rutest=tt_grplabel)),PSFORMAT=&g_trtcd &g_trtfmt ruorrscd $yesno.), /* Same as SEGMENT12 */
   SEGMENT6            =%str(dsetinnumer=ardata.rucam(where=(rutestcd eq 'LIMGPERF')),DSETOUT=segment6_out(rename=(ruorrscd=tt_code1 ruorres=tt_decode1 rutest=tt_grplabel)),PSFORMAT=&g_trtcd &g_trtfmt ruorrscd $yesnoyn.), /* Same as SEGMENT12 */
   SEGMENT7            =%str(dsetinnumer=ardata.rucam(where=(rutestcd eq 'LIMGNORM')),DSETOUT=segment7_out(rename=(ruorrscd=tt_code1 ruorres=tt_decode1 rutest=tt_grplabel)),PSFORMAT=&g_trtcd &g_trtfmt ruorrscd $yesnoyn.), /* Same as SEGMENT12 */
   SEGMENT8            =%str(dsetinnumer=ardata.rucam(where=(rutestcd eq 'BIOPANY' )),DSETOUT=segment8_out(rename=(ruorrscd=tt_code1 ruorres=tt_decode1 rutest=tt_grplabel)),PSFORMAT=&g_trtcd &g_trtfmt ruorrscd $yesnoyn.), /* Same as SEGMENT12 */
   SEGMENT9            =%str(dsetinnumer=ardata.rucam(where=(rutestcd eq 'UCVMDANY')),DSETOUT=segment9_out(rename=(ruorrscd=tt_code1 ruorres=tt_decode1 rutest=tt_grplabel)),PSFORMAT=&g_trtcd &g_trtfmt ruorrscd $yesnoyn.), /* Same as SEGMENT12 */
   SEGMENT10           =%str(dsetinnumer=ardata.rucam(where=(rutestcd eq 'FSTDTYCH')),DSETOUT=segment10_out(rename=(ruorrscd=tt_code1 ruorres=tt_decode1 rutest=tt_grplabel)),PSFORMAT=&g_trtcd &g_trtfmt ruorrscd $yesnoyn.), /* Same as SEGMENT12 */
   ...
   DDDATASETLABEL      =DD dataset for new LIVER1 table, /* Label to be applied to the DD dataset */
   ...
   FORMATS             =tt_decode1 $rustrpt. tt_grplabel, /* Format specification */
   IDVARS              =tt_code1 tt_grplabel tt_decode1 tt_summaryLevel, /* Variables to appear on each page should the report be wider than 1 page */
   ...
   POSTSUBSET          =if tt_code1='Y' then tt_code1='A'; if tt_segorder gt 2 then tt_decode1='  '||tt_decode1, /* SAS expression to be applied to data immediately prior to creation of the permanent presentation dataset */
   ...
   XMLDEFAULTS         =&g_refdata/tr_liver1_defaults.xml /* Location and name of XML defaults file */
   );
   ...
   %if &l_flag %then
   %do;
      libname _liver1 xml "&xmldefaults";
      proc sql noprint;
         select count(memname) into :l_flag
         from dictionary.tables
         where libname eq '_LIVER1' and memname eq 'LIVER1'
         ;
      quit;
      %if not &l_flag %then
      %do;
         libname _liver1 clear;
      %end;
   %end;
