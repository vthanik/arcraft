...
|                    - To be used for Cross Over Study Design
|
...
|                           7  A validation check needs to be included to check if 
|                              PERNUM exists in the TMSLICE dataset. If NOT, the macro 
|                              should halt processing and an RTERROR message be 
|                              included in the SAS Log indicating this.
|                           8  Include in the merge statements, a conditional 
|                              merge with variable TPTREFN/PTMNUM if it exists. 
|                           9  If EXSTTM is not included in exposuredset, then  
|                              create a dummy variable setting the values to missing 
|                           10 Included EXPOSUREDSET and TMSLICEDSET to be passed 
...
%macro tc_p1_ae_xo(
  ...
  %let listvars=DSETIN_AE DSETOUT DSETIN_VITALS EXPOSUREDSET TMSLICEDSET;
  ...
  * DMDATA.TMSLICE exists and variable check;
  ...
  %let prefix = _tc_p1_ae_xo ;   /* Root name for temporary work datasets */
  ...
  *   message is issued to the log                      ;
  ...
  /*-- Checck if use tu_p1_calctpernum or tu_calctpernum macro */
 ...
 /----------------------------------------------------------------------------*/
  %tc_ae ( dsetin            = &preprocessdset /* Input dataset name */
          ...
          ,xovarsforpgyn     = &xovarsforpgyn  /* If derive crossover stydy specific variables for parallel study */
  ...
  %do ;
    ...
    data &prefix._ae1 (DROP=tpernum tperiod) ;
      SET &prefix._ae1;
    run;
  ...
  * Set a temporary macro variable containing dataset reference ;
  ...
  %do;
    ...
    /---------------------------------------------------------------------------*/
    %tu_rantrt ( dsetin      = &prefix._ae2   /* Input dataset name */
    ...
    /---------------------------------------------------------------------------*/
    %tu_p1_acttrt_xo ( dsetin         = &prefix._ae3    /* Name of input dataset */
                      ,dsetout        = &prefix._ae4    /* Name of output dataset */
    ...
    * Set a temporary macro variable containing dataset reference ;
    %let postprocessdset = &prefix._ae4;
  ...
  %tu_p1_periodday ( dsetin         =&postprocessdset  /* Input dataset                                    */
                    ,dsetout        =&prefix._ae5      /* Output dataset                                   */
                    ,refdate        =aestdt            /* Reference date variable on input dataset         */
  ...
  /---------------------------------------------------------------------------*/
  %tu_p1_periodday ( dsetin         =&prefix._ae5  /* Input dataset                                    */
                    ,dsetout        =&prefix._ae6  /* Output dataset                                   */
                    ,refdate        =aeendt        /* Reference date variable on input dataset         */
                    ,eventtype      =SP            /* Specifies if the event is planned or spontaneous */
  ...
  /*
  / Get unique dosing timepoint from EXPOSURE and TMSLICE
  /---------------------------------------------------------------------------*/
  data &prefix._expo1 ;
    set &exposuredset ;
  ...
  *   observation containing time for the same subject, date    ;
  ...
  data &prefix._expo3 (keep=subjid pernum exstdm exstdt tempdat);
    merge &prefix._expo2 (in=a)
          &prefix._tmslice;
    by &bylist ;
    if a;
  ...
  proc sort data = &prefix._expo3
    ...
    by subjid pernum exstdt tempdat exstdm;
  run;

  /*
  / slot the AE and EXPO datasets to derive the
  /   - Time from First Dose        (AETRT1ST / AETRT1SC)
  /   - Time from First Period Dose (AEPTR1ST / AEPTR1SC)
  /   - Time from Last Dose and     (AETRTST  / AETRTSC )
  ...
  proc sort data=&prefix._ae7;
    by subjid tpernum aestdt tempdat aestdm;
  run;

  DATA &prefix._ae8 ;
    SET &prefix._expo4 (IN=expo RENAME=(pernum=tpernum exstdt=aestdt exstdm=aestdm))
        &prefix._ae7   (IN=ae);
    BY subjid tpernum aestdt tempdat aestdm;
    FORMAT first_dose_m period_dose_m last_dose_m datetime20.
           first_dose_d period_dose_d last_dose_d date9.       ;

    RETAIN first_dose_m period_dose_m last_dose_m 
           first_dose_d period_dose_d last_dose_d;

    IF FIRST.subjid THEN DO;
      first_dose_m =. ; first_dose_d = . ;
      last_dose_m  =. ; last_dose_d  = . ;
    END;
    IF FIRST.tpernum THEN DO; 
      period_dose_m =.; period_dose_d = . ;
    END;
    IF expo then do;
      IF first_dose_m  = . THEN first_dose_m  = aestdm;
      IF first_dose_d  = . THEN first_dose_d  = aestdt;
      IF period_dose_m = . THEN period_dose_m = aestdm;
      IF period_dose_d = . THEN period_dose_d = aestdt;
                                last_dose_m   = aestdm;
                                last_dose_d   = aestdt;
    END;
  ...
  DATA &prefix._ae9;
    ...
    * If AE start time is not missing ;
    IF aesttm NE . THEN
    DO ;
      ...
      * Calculate Time since Period First Dose ;
      if period_dose_m ne . then
      do ;
        %tu_times(unit=m, start=period_dose_m, end=aestdm, output=aeptr1st, outputc=aeptr1sc);
      end;
      else 
      do;
        %tu_times(unit=d, start=period_dose_d, end=aestdt, output=aeptr1st, outputc=aeptr1sc);
      end;
  ...
  *   check whether to include vitals processing or not  ;
  ...
  %do ;
    ...
    DATA &prefix._ae10;
      MERGE &prefix._ae9(IN=a)
            &vitalsSubsetdset;
      BY subjid;
      IF a;
    RUN;

    %let postprocessdset = &prefix._ae10 ;
