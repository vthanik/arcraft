...
|                     of subjects who completed the study as well as subjectrs who 
|                     withdrew prematurely from the study will be summarized by 
|                     completion status and reason for withdrawal (ES1)
|                     
...
|                     appropriate parameters for              the  rsNumer=&g_trtcd
|                     macro to create an output dataset. The       dsfail,DSETOUT=seg
|                     percentage sign shall not be used as a       ment1_out(rename=(
|                     prefix to the macro name, and the            dsfail=tt_code1)),
|                     parameters shall not be surrounded by        ROWLABELVARNAME=tt
|                     brackets, i.e. the following style shall     _grplabel='Complet
|                     be used:  segment1=MacroName                 ion Status')
...
|                     and variable names                           dsrscd tt_code1
...
|                     that are given the PROC REPORT define        dsrscd tt_code1
...
|                     syntactically valid SAS where or if          for withdrawl';
|                     statement for use in a data step             end; else
|                                                                  tt_decode1=put(tt_
|                                                                  code1, $dsfail.))
|
...
|
| XMLDEFAULTS         Specifies a XML file which defined           &g_refdata/tr_es1a
...
%macro td_es1a (
   SEGMENT1            =%str(dsetinnumer=ardata.ds,groupByVarsNumer=&g_trtcd dsfail,DSETOUT=segment1_out(rename=(dsfail=tt_code1)),ROWLABELVARNAME=tt_grplabel='Completion Status'), /* Same as SEGMENT4 */
   SEGMENT2            =%str(dsetinnumer=ardata.ds(where=(dsfail eq 'Y')), groupByVarsNumer=&g_trtcd dsrscd, DSETOUT=segment2_out, POSTSUBSET=if 1 then tt_decode1=dsrs), /* Same as SEGMENT4 */
   SEGMENT3            =%str(dsetinnumer=ardata.ds(where=(dsfail eq 'Y' and not missing(dssbrscd))),DSETOUT=segment3_out(rename=(dssbrscd=tt_code1)),POSTSUBSET=if 1 then tt_decode1='  '||dssbrs), /* Same as SEGMENT4 */
   ...
   COLUMNS             =tt_segorder tt_grplabel dsrscd tt_code1 tt_decode1 tt_ac:, /* Column parameter */
   ...
   DDDATASETLABEL      =DD dataset for new ES1 table, /* Label to be applied to the DD dataset */
   ...
   NOPRINTVARS         =tt_segorder dsrscd tt_code1, /* No print vars (usually used to order the display) */
   NOWIDOWVAR          =dsrscd,            /* Variable whose values must be kept together on a page */
   ORDERDATA           =,                  /* ORDER=DATA variables */
   ORDERFORMATTED      =,                  /* ORDER=FORMATTED variables */
   ORDERFREQ           =,                  /* ORDER=FREQ variables */
   ORDERVARS           =tt_segorder tt_grplabel dsrscd tt_code1 tt_decode1, /* Order variables */
   PAGEVARS            =,                  /* Break after <var> / page; */
   POSTSUBSET          =%nrstr(if tt_segorder ge 2 then do; tt_segorder=2; tt_grplabel='Primary[1]/subreason[2] for withdrawl'; end; else tt_decode1=put(tt_code1, $dsfail.)), /* SAS expression applied before call %tu_list */
   ...
   XMLDEFAULTS         =&g_refdata/tr_es1a_defaults.xml /* Location and name of XML defaults file */
   );
   ...
   %if &l_flag %then
   %do;
      libname _tdes1a xml "&xmldefaults";
      proc sql noprint;
         select count(memname) into :l_flag
         from dictionary.tables
         where libname eq '_TDES1A' and memname eq 'ES1A'
         ;
      quit;      
      %if not &l_flag %then
      %do;      
         libname _tdes1a clear;
      %end; 
   %end;
