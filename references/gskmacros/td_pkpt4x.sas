...
|                    Table (PKPT4X) as defined in the ISDL Standard Data Displays document.
|                    This macro is designed to report data from Cross-Over studies.
...
|
|----------------------------------------------------------------------------------------------------
| ACROSSVAR                Passed as %tu_sumstatsinrows parameter of the same name.  &g_ptrtcd (Req)           
|
| ACROSSVARDECODE          Passed as %tu_sumstatsinrows parameter of the same name.  &g_ptrtgrp  (Req)           
|
...
|
| CENTREVARS               Passed as %tu_sumstatsinrows parameter of the same name.  (blank) (Opt)
|
| CODEDECODEVARPAIRS       Passed as %tu_sumstatsinrows parameter of the same name.  &g_ptrtcd &g_ptrtgrp (Req)
| 
| COLSPACING               Passed as %tu_sumstatsinrows parameter of the same name.  1 (Req)
|
| COLUMNS                  Passed as %tu_sumstatsinrows parameter of the same name.  pppar tt_svid tt_svnm tt_p: 
|
| COMPLETETYPESVARS        Passed as %tu_sumstatsinrows parameter of the same name.  &g_ptrtcd &g_ptrtgrp,   
|
...
|
| DDDATASETLABEL           Passed as %tu_sumstatsinrows parameter of the same name.  DD Dataset for PKPT4X (Req)
|
| DEFAULTWIDTHS            Passed as %tu_sumstatsinrows parameter of the same name.  pppar 12 tt_svnm 15 tt_p: 20
|
...
|
| GROUPBYVARPOP            Passed as %tu_sumstatsinrows parameter of the same name.  &g_ptrtcd (Req)
|
| GROUPBYVARSANALY         Passed as %tu_sumstatsinrows parameter of the same name.  pppar &g_ptrtcd &g_ptrtgrp (Req)
|
...
|
| NOPRINTVARS              Passed as %tu_sumstatsinrows parameter of the same name.  tt_svid (Req)
|
| NOWIDOWVAR               Passed as %tu_sumstatsinrows parameter of the same name.  (blank) (Opt)  
|
...
|
| ORDERVARS                Passed as %tu_sumstatsinrows parameter of the same name.  pppar tt_svid tt_svnm (Req)
|
...
|
| SKIPVARS                 Passed as %tu_sumstatsinrows parameter of the same name.  pppar (Req)
|
...
|    
| STATSDPS                 Passed as %tu_sumstatsinrows parameter of the same name.  geomeanclm +1 meanclm +1 std +2 
|                                                                                    stdlog +2 median +1 (Req)
|
| STATSINROWSYN            Passed as %tu_sumstatsinrows parameter of the same name.  Y (Req)
|
| STATSLABELS              Passed as %tu_sumstatsinrows parameter of the same name.  %nrstr(meanclm='Arith. Mean~  95% CI' 
|                                                                                    geomeanclm='Geom. Mean~  95% CI' 
|                                                                                    stdlog='SD Logs' cvblog='%CVb') (Req)
|
| STATSLIST                Passed as %tu_sumstatsinrows parameter of the same name.  n geomeanclm stdlog meanclm 
|                                                                                    std cvlog median min max  (Req) 
|
...
| Date of Modification :     04-Aug-05
| New Version Number :       01-002
| Modification ID :          DIS.01.002
| Reason For Modification :  DIS.01.002.a: Modifications to header from SCR
|                            DIS.01.002.b: Add calls to tu_getdata and tu_nobs
|                                          to allow subsetting
|                           
...
|
| Modified By :              Greg Weber
| Date of Modification :     17-Aug-05
| New Version Number :       01-004
| Modification ID :          GTW.01.004
| Reason For Modification :  GTW.01.004.a: Remove visit columns from defaults       
|
| Modified By :
...
%MACRO td_pkpt4x(
   ACROSSVAR                =&g_ptrtcd,            /* Variable that will be transposed to columns*/
   ACROSSVARDECODE          =&g_ptrtgrp,           /* The name of a decode variable for ACROSSVAR, or the name of a format*/
   ...
   CODEDECODEVARPAIRS       =&g_ptrtcd &g_ptrtgrp, /* Code and Decode variables in pairs*/
   COLSPACING               =1,                    /* Overall spacing value. */
   COLUMNS                  =pppar tt_svid tt_svnm tt_p:,/* Column parameter */
   ...
   COMPLETETYPESVARS        =&g_ptrtcd &g_ptrtgrp, /* Variables which COMPLETETYPES should be applied to */ 
   DDDATASETLABEL           =DD dataset for PKPT4X,/* Label to be applied to the DD dataset */
   DEFAULTWIDTHS            =pppar 12 tt_svnm 15 tt_p: 20, /* List of default column widths */
   ...
   DSETIN                   =ardata.pkpar,         /* type:ID Name of input analysis dataset*/
   DSETOUT                  =,                     /* Output summary dataset*/
   FLOWVARS                 =_ALL_,                /* Variables with flow option */
   FORMATS                  =,                     /* Format specification */
   GROUPBYVARPOP            =&g_ptrtcd,            /* Variables to group by when counting big N */
   GROUPBYVARSANALY         =pppar &g_ptrtcd &g_ptrtgrp, /* The variables whose values define the subgroup combinations for the analysis */
   ...
   NOPRINTVARS              =tt_svid,              /* No print variables (usually used to order the display) */
   ...
   ORDERVARS                =pppar tt_svid tt_svnm,/* Order variables */
   ...
   SKIPVARS                 =pppar,                /* Break after <var> / skip */
   ...
   STATSDPS                 =geomeanclm +1  meanclm +1 std +2 stdlog +2 median +1,  /* Number of decimal places of summary statistical results*/
   STATSINROWSYN            =Y,                    /* Should summary statistics be displayed in rows or columns */
   STATSLIST                =n geomeanclm stdlog meanclm std cvblog median min max, /* List of required summary statistics, e.g. N Mean Median. */
   STATSLISTVARNAME         =tt_svnm,              /* Variable name that saves summary statistic variable names */
   STATSLABELS              =%nrstr(meanclm='Arith. Mean~  95% CI' geomeanclm='Geom. Mean~  95% CI' stdlog='SD Logs' cvblog='%CVb'), /* Label defination for summary statistical result variables. */
   ...
   /*
   / Echo macro parameters to log file       
   /----------------------------------------------------------------------------*/   
