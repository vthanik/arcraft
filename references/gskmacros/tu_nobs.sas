...
| Date: 30May2003
|
| Macro Purpose: A macro to count the number of observations in a dataset
|
...
|
|
| Output: specifies the number of physical observations (including
|         those that are marked for deletion). An active WHERE clause
...
%macro tu_nobs(
        dsetin    /* the input dataset for which the observation count is required  */
        );
