...
*
*
*  Macro purpose: display macro to generate IDSL es3 listing
*
...
*
*  dsetin          The study conclusion data set to act as the subject
*                  of the report. Valid values: name of a data set
*                  meeting the IDSL dataset specification for study
*                  conclusion data                                     ardata.disposit
...
*                  dataset                                            %str(upcase(substr(dswd, 1, 1)) EQ 'Y')
...
*                  string                                              DD dataset for ES3 listing
...
*                  COLUMN statement syntax                             &g_centid &g_subjid &g_trtgrp dsacttrt dsdt dsactdy dsperdy esreason    
...
*                  width of  column header)                            &g_trtgrp dsacttrt dsdt esreason
...
*                  width, width of column header)                      &g_centid 6 &g_subjid 6 &g_trtgrp 10 dsacttrt 10 dsdt 10 dsactdy 5 dsperdy 6 esreason 30
...
*                  Valid values: one or more variable names from
*                  DSETIN that are also defined with COLUMNS           dsdt
...
%macro td_es3(
   dsetin         =ardata.disposit, /* Input study conclusion dataset */
   ...
   presubset      =%str(upcase(substr(dswd, 1, 1)) EQ 'Y'), /* SAS IF condition (without IF) that applies to the input dataset. */
   varlabelstyle  = SHORT,    /* Specifies the label style for variables (SHORT or STD)  */
   dddatasetlabel=DD dataset for ES3 listing, /* Label to be applied to the DD dataset  */
   ...
   columns        =&g_centid &g_subjid &g_trtgrp dsacttrt dsdt dsactdy dsperdy esreason  , /* Columns to be included in the listing (plus spanned headers)  */
   ...
   byvars         =,          /* By variables */
   flowvars       =&g_trtgrp dsacttrt dsdt esreason, /* Variables with flow option */
   widths         =,          /* Column widths */
   defaultwidths  =&g_centid 6 &g_subjid 6 &g_trtgrp 10 dsacttrt 10 dsdt 10 dsactdy 5 dsperdy 6 esreason 30, /* List of default column widths */
   ...
   rightvars      =dsdt,      /* Right justify variables  */
 ...
 /* verify data and variables exist below   */
   ...
   %if %length(%tu_chkvarsexist(&l_workdata, dsreascd dsrsoth dsreas)) ne 0 %then
   %do;
 ...
 proc sql;
    create table &l_prefix.es32 as select *,
      case (upcase(dsreascd))
           when 'Z' then 'Other: ' !! dsrsoth
           else dsreas
           end
        as esreason label='Reason for Premature Withdrawal'
      from &l_workdata;
 quit;

 %let l_workdata=&l_prefix.es32;
