...
| Date:               04aug2003
|                     
| Macro purpose:      display macro to generate IDSL cm2 listing
|                     
...
|                     Including spanning titles and variable names      ST_CM_CS2 cmstdt  
|                     Valid values: one or more variable names from     ST_CM_ADT2        
|                     DSETIN plus other elements of valid PROC REPORT   ST_CM_SA2 cmend_  
|                     COLUMN statement syntax                           cmongo            
|                                                                                         
...
|                                                                                         
| DEFAULTWIDTHS       This parameter specifies default column widths    ST_CM_CS2 10       
|                     for all variables not listed in the WIDTHS        ST_CM_ADT2 34     
|                     parameter.                                        ST_CM_SA2 15      
|                     Valid values: values of column names and numeric  cmend_ 12 cmongo 10 
...
|                                                                                         
| DSETIN              The concomitant medications data set to act as                      
|                     the subject of the report.                        Ardata.cmanals(whe
|                     Valid values: name of a data set meeting the      re=(display1 eq   
|                     IDSL dataset specification for concomitant        1))               
|                     medications data                                  
|                                                                                         
...
|                     defined as NOPRINT                                                  
|                                                                                         
| NOPRINTVARS         Variables listed in the COLUMN parameter that     &g_trtcd cmstdt
...
|                     REPORT define statement attribute ORDER           ST_CM_CS2 cmstdt
|                     Valid values: one or more variable names from     ST_CM_ADT2 st_cm_sa2      
|                     DSETIN that are also defined with COLUMNS         cmend_ cmongo                  
|                                                                                         
...
|                     Valid values: positive integer                                      
|                                                                                         
| SKIPVARS            Variables whose change in value causes the        ST_CM_ADT2        
...
|                     regarding macro parameters that can be used in    varout 
|                     the macro call.  Note that the DSETIN parameter   =ST_CM_CS2, sepc 
...
|                                                                       / )               
|                                                                                         
| STACKVAR4-          Specifies any variables that should be stacked    No default        
...
|                           6.ST_CM_SA2 CMEND_ and CMONGO added to ORDERVARS
|                           7.Changed all CMSTDT to CMSTD_
|-----------------------------------------------------------------------------------------
| Modified by:              Yongwei Wang (YW62951)
...
%macro td_cm2(
   ...
   COLUMNS             =&g_trtcd ST_CM_CS2 cmstdt ST_CM_ADT2 ST_CM_SA2 cmend_ cmongo, /* Columns to be included in the listing (plus spanned headers) */                                                                                                     
   ...
   DDDATASETLABEL      =DD dataset for CM2 listing, /* Label to be applied to the DD dataset */                                                                                                                                                              
   DEFAULTWIDTHS       =st_cm_cs2 10 st_cm_adt2 34 st_cm_sa2 15 cmend_ 12 cmongo 10, /* List of default column widths */
   ...
   NOPRINTVARS         =&g_trtcd cmstdt,   /* No print vars, used to order the display */                                                                                                                                                                    
...
ORDERVARS           =&g_trtcd ST_CM_CS2 cmstdt ST_CM_ADT2 st_cm_sa2 cmend_ cmongo,
/* Order variables */
   ...
   SKIPVARS            =ST_CM_ADT2,        /* Variables whose change in value causes the display to skip a line */                                                                                                                                           
   SPLITCHAR           =~,                 /* Split character */                                                                                                                                                                                             
   STACKVAR1           =%str(varsin=&g_centid &g_subjid,varout=ST_CM_CS2,sepc=/), /* Create stacked variables (e.g., stackvar1=%str(varsin= invid subjid, varout= st_inv_subj, sepc=/, splitc=~) ) */                                    
   STACKVAR2           =%str(varsin=cmatc1 cmcomp cmterm,varout=ST_CM_ADT2,sepc=/), /* Create stacked variables (e.g., stackvar2=%str(varsin= invid subjid, varout= st_inv_subj, sepc=/, splitc=~) ) */                                              
   STACKVAR3           =%str(varsin=cmstd_ cmactsdy,varout=ST_CM_SA2,sepc=/), /* Create stacked variables (e.g., stackvar3=%str(varsin= invid subjid, varout= st_inv_subj, sepc=/, splitc=~) ) */                                                     
