...
*  Date:           04aug2003
*
*  Macro purpose:  display macro to generate IDSL bl2 listing
*
...
*------------------------------------------------------------------------------------
*  dsetin          The status of treatment blind dataset to act 
*                  as the subject of the report. Valid values: name 
*                  of a data set meeting the IDSL dataset 
*                  specification for AE data                           ardata.blind
...
*                  provided here.                                      %str(varsin=&g_trtgrp blacttrt, varout=st_bl_ta2, sepc=/)
*  stackvar2       Additional stacked variables (ref: stackvar1)       blank
*  - stackvar15    
...
*                  string                                              DD dataset for BL2 listing
...
*                  COLUMN statement syntax                             &g_centid &g_subjid st_bl_ta2 bldt blactdy blperdy blreason
...
*                  width of coulmn header).                            st_bl_ta2 blreason
...
*                  width, width of column header)                      blreason 30 blactdy 5 blperdy 6
...
*                  Valid values: one or more variable names from
*                  DSETIN that are also defined with COLUMNS           bldt
...
%macro td_bl2(
   dsetin         =ardata.blind, /* Input treatment blind dataset */
   stackvar1      =%str(varsin=&g_trtgrp blacttrt, varout=st_bl_ta2, sepc=/),  /* Create stacked variables (e.g., stackvar1=%str(varsin=invid subjid, varout=st_inv_subj,sepc=/,splitc=~))       */
   ...
   dddatasetlabel=DD dataset for BL2 listing,  /* Label to be applied to the DD dataset                        */ 
   ...
   columns        =&g_centid &g_subjid st_bl_ta2 bldt blactdy blperdy blreason , /* Columns to be included in the listing (plus spanned headers) */
   ...
   byvars         =,          /* By variables */
   flowvars       =st_bl_ta2 blreason,    /* Variables with flow option */
   widths         =,          /* Column widths */
   defaultwidths  =blreason 30 blactdy 5 blperdy 6, /* List of default column widths */
   skipvars       =&g_centid, /* Variables whose change in value causes the display to skip a line  */
   ...
   leftvars       =bldt,      /* Left justify variables   */ 
 ...
 %let l_prefix=_td3;
   ...
   /*  Verify that the input dataset and any variables involved in pre-processing exist: */
  %if %length(%tu_chkvarsexist(&dsetin, blreascd blreas blrsoth)) ne 0 %then
  %do;
    %put RT%str(ERR)OR: &sysmacroname: Aborting due to errors noted above;
    %tu_abort(option=force);
  %end;

 /* Concatenate variables to show description of */

 proc sql;
    create table &l_prefix._bl2 as
    select *, 
      case (upcase(blreascd))
           when 'Z' then 'Other: ' !! blrsoth
           else blreas
           end
           as blreason label='Reason' 
      from &dsetin. ;
 quit;
 
 /*  call tu_list below */    

 %tu_list(dsetin=&l_prefix._bl2,
    stackvar1      =&stackvar1 ,
    stackvar2      =&stackvar2 ,
    stackvar3      =&stackvar3  ,
    stackvar4      =&stackvar4 ,        
