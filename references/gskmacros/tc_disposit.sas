...
| Date:            26-Jun-2004
|
| Macro Purpose:   Disposit wrapper macro
|
...
| -----------------  -------------------------------------  -------  ----------
| DSETIN             Specifies the DISPOSIT-format SI       REQ      DMDATA.DISPOSIT
|                    dataset which needs to be transformed 
|                    into a DISPOSIT-format A&R dataset.
|                    Valid values: valid dataset name
|
| DSETOUT            Specifies the name of the output       REQ      ARDATA.DISPOSIT
...
|                    derivations for this domain code (DS)?
|                    Valid values: Y, N
|
...
|                    VISIT based on the disposition date?
|                    Valid values: Y, N          
|
...
|                    Valid values: Y, N
|
| REFDAT             Specify a reference date variable name OPT      DSDT
|                    to pass to %tu_recalcvisit to  
|                    calculate the visit. Will be checked  
|                    in %tu_recalcvisit
|
| REFTIM             Specify a reference time variable name OPT      DSWDTM
|                    to pass to %tu_recalcvisit to  
|                    calculate the visit. Will be checked  
|                    in %tu_recalcvisit
|
...
|                    (i.e. left to its default value), or
|                    is specified as anything other than 
|                    blank, then DSETTEMPLATE, SORTORDER 
|                    and FORMATNAMESDSET must not be
|                    specified as anything non-blank. If
|                    DSETTEMPLATE, SORTORDER and
|                    FORMATNAMESDSET are specified as 
|                    anything non-blank, then DSPLAN must 
|                    be specified as blank (DSPLAN=,).
|
...
|                    must be specified as blank (DSPLAN=,).
|
...
| Date of Modification:     24-Aug-2005
| New version/draft number: 2/1
| Modification ID:          YW001
| Reason For Modification:  1. Added three parameters REFDAT, REFTIM and 
|                              RECALCVISITYN
|                           2. Added check of RECALCVISITYN and REFTIM
|                           3. Added call of %tu_recalvisit
|
| Modified By:              Yongwei Wang
...
%macro tc_disposit (
 ...
 %local prefix;
 %let prefix = _tc_disposit;   /* Root name for temporary work datasets */
 ...
 /*
 / YW001: Recalculate visit based on &REFDAT and &REFTIM.
 /----------------------------------------------------------------------------*/
 ...
 %do;
    %tu_derive (
         dsetin            = &prefix._ds&i,
         dsetout           = &prefix._ds%eval(&i+1),
         domaincode        = ds,                     /* Domain Code - type of dataset */
