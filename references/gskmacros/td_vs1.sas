...
| Date              : 28-Jul-03
|
| Macro Purpose     : display macro to generate IDSL vs1 table
|
...
|                     TU_STATSFMT[9]
|
| ANALYSISVARS        The variables to be analysed.                     vsstresn
...
|
| CODEDECODEVARPAIRS  Specifies code and decode variable pairs. These   vstestcd vstest 
|                     variables should be in parameter                  &g_trtcd &g_trtgrp
|                     GROUPBYVARSANALY. The first variable in the pair  visitnum visit
...
|
| DSETIN              The VITALS data set to act as the subject of the  ardata.vsanal
|                     report
|                     Valid values: name of a data set meeting the
|                     IDSL dataset specification for VITALS data
|
| DSETOUT             Specifies the name of the output                 
|                     dataset to be created.
|                     Valid values: valid dataset name 
|                     
| FLOWVARS            Variables to be defined with the flow option in   vstest
|                     PROC REPORT                                       &g_trtgrp
|                     Valid values: one or more variable names from     visit
|                     DSETIN that are also defined with COLUMNS
...
|                     %tu_getdata
|
| GROUPBYVARSANALY    Specifies the variables whose values define the   vstestcd vstest 
|                     subgroup combinations for the analysis. The       &g_trtcd &g_trtgrp
|                     variables can be divided by statements inside of  visitnum visit
...
|                     type as variable.
|
| IDVARS              Variables to appear on each page if the report    vstestcd vstest
...
|                     DSETIN that are also defined with COLUMNS
|
| LABELS              Variables and their label for display. For use    vstest='~'
...
|                     defined as NOPRINT
|
| NOPRINTVARS         Variables listed in the COLUMN parameter that     vstestcd &g_trtcd
...
|                     attribute order=internal
|
| ORDERVARS           List of variables that will receive the PROC      vstestcd vstest
...
|                     Valid values: positive integer
|
| SKIPVARS            Variables whose change in value causes the        vstest &g_trtgrp
...
|                     provided here
|
| STATSDPS            Specifies decimal places of statistical results   MEDIAN +1
|                     of analysis variables.                            MEAN +1
|                     If the decimal positions for all variables given  STD +2
...
|
| STATSLIST           Specifies a list of summary statistics to         N MIN
|                     produce. May also specify correct PROC SUMMARY    MAX MEDIAN
|                     syntax to rename output variable (N=number        STD MEAN
|                     MEAN=average)
...
|
| WIDTHS              Variables and width to display                    vstest 13
...
| Date of Modification :    20-Nov-2006
| New Version Number :      3/1
| Modification ID :         BA001
| Reason For Modification : 1. Add new parameters DISPLAY and DSETOUT, change request HRT0091
|                           2. Change default for DSETIN to ardata.VSANAL, change request HRT0121
|                           3. Add POSTSUBSET parameter to tu_sumstatsinrows, change request HRT0113
|                           4. Change macro vars tt_avnm to vstest AND tt_avid to vstestcd, 
|                              change request HRT0121
|
+-------------------------------------------------------------------------------------------------*/
...
%macro td_vs1(        
   ...
   ANALYSISVARS            =vsstresn, /*Summary statistics analysis variables*/
   ...
   CODEDECODEVARPAIRS      =vstestcd vstest &g_trtcd &g_trtgrp visitnum visit, /* Code and Decode variables in pairs */
   COLSPACING              =2,     /*Value for between-column spacing*/
   COLUMNS                 =vstestcd vstest &g_trtcd &g_trtgrp tt_bnnm visitnum visit N MEAN STD MEDIAN MIN MAX, /*Columns to be included in the listing (plus spanned headers)*/
   ...
   DDDATASETLABEL          =DD dataset for VS1 table, /*DD dataset for a table	Label to be applied to the DD dataset*/
   DEFAULTWIDTHS           =,	   /*List of default column widths*/
   DESCENDING              =,	   /*Descending ORDERVARS*/
   DISPLAY                 =Y,     /* Specify Y or N to create the display */
   DSETIN                  =ardata.vsanal, /*Input analysis dataset*/   
   DSETOUT                 =,      /* One or two-level name of output dataset */
   FLOWVARS                =vstest &g_trtgrp visit, /*Variables with flow option*/
   FORMATS                 =,      /*Format specification (valid SAS syntax)*/
   GROUPBYVARPOP           =&g_trtcd, /*Variables to group by when counting big N */
   GROUPBYVARSANALY        =vstestcd vstest &g_trtcd &g_trtgrp visitnum visit, /*The variables whose values define the subgroup combinations for the analysis*/
   IDVARS                  =vstestcd vstest &g_trtcd &g_trtgrp tt_bnnm visitnum visit, /*Variables to appear on each page of the report*/
   LABELS                  =vstest='~', /*Label definitions (var="var label")*/
   LEFTVARS                =,      /*Left justify variables*/
   LINEVARS                =,      /*Order variables printed with LINE statements*/
   NOPRINTVARS             =vstestcd &g_trtcd visitnum, /*No print variables, used to order the display*/
   ...
   ORDERVARS               =vstestcd vstest &g_trtcd &g_trtgrp tt_bnnm visitnum visit, /*Order variables*/
   ...
   SKIPVARS                =vstestcd &g_trtgrp, /*Variables whose change in value causes the display to skip a line */
   ...
   WIDTHS                  =vstest 13 &g_trtgrp 11 visit 9 tt_bnnm 3 /*Column widths*/              
   );         
