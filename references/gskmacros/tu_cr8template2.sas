...
|                  creating output with two plots on each page. Panes
|                  shall be provided for header/footer, by-line, legend,
|                  and 2 plots (side-by-side). The first three shall be 
|                  the full size of the display. The latter shall be sized
|                  to fit the available/specified space.
|
...
| Date of Modification:     10-May-05
...
|
*******************************************************************************/
%macro tu_cr8template2(
  TC              /* Specifies the output graphic catalog */
 ...
 ,framePlot = N   /* Place frame around plot(s)?          */
  ...
  %local sideMargin midMargin;
  %let sideMargin=0;  /* Minimum percent of page to be left as white space at left and right */
  %let midMargin=2.5; /* Minimum percent of page to be left as white space between plots */

  /*
  / Firstly, we need to establish the coordinates 
  / for the graph panes. Need to do non-integer   
  / arithmetic, so use DATA step (macros cannot   
  / do non-integer arithmetic).                   
  /------------------------------------------------------*/

  %local Size LeftX RightX LowerY UpperY XlateX;

  data _null_;
    maxPlotWidth=(100-2*&sideMargin.-&midMargin.)/2;
    maxvert=&topy.-&boty.;
    ...
    do;
      ...
      xlatex=maxPlotWidth+&midMargin.;
    end;
    else 
    do;  
      /*
      / TOPY-BOTY is less than or equal to maxPlotWidth, so the size of the 
      / graph panels shall be TOPY-BOTY.                        
      /------------------------------------------------------*/
      size=maxvert;
      gap=(50-&sideMargin.-size)/2;
      leftx=&sideMargin.+gap;
      lowery=&boty.;
      xlatex=size+2*gap;
    end;

    call symput('Size',compress(putn(size,'BEST.')));
    call symput('LeftX',compress(putn(leftx,'BEST.')));
    call symput('RightX',compress(putn(leftx+size,'BEST.')));
    call symput('LowerY',compress(putn(lowery,'BEST.')));
    call symput('UpperY',compress(putn(lowery+size,'BEST.')));
    call symput('XlateX',compress(putn(xlatex,'BEST.')));

  run;

  %if &g_debug ge 1 %then
  %do;
    %put RTD%str(EBUG): &sysmacroname: SIZE=&size., LEFTX=&leftx., RIGHTX=&rightx., LOWERY=&lowery., UPPERY=&uppery., XLATEX=&xlatex.;
  %end;
