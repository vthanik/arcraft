...
| Date:            26-April-2011
|
| Macro Purpose:   Create EG7 display using TIBCO Spotfire Clinical Graphics (TSCG)
|
...
|
********************************************************************************************/
%macro td_eg7tscg(InputUsage     = ,        /* Style of input data: C=Create pre-processing program, D=Use existing dataset as input for template, P=Call pre-processing program, U=Use template */
                  InputFile      = ,        /* Name of pre-processing program located in refdata folder to either create or read in. No folder path is required - only the file name - e.g. cr8_eg7tscg.sas  */
                  dataset        = ,        /* Name of input dataset for IGD file. Note this will be the name of the processed dataset after calling pre-processing program.  2-level name required - e.g. dddata.eg7ae10tscg */
                  igdFile        = ,        /* Name of IGD file to call located in refdata folder. No folder path is required - only the file name - e.g. eg7tscg.igd  */
  ...
  * Parameter cleanup;
  ...
  %macro splusPlot_write_template(fname);
    ...
    data _null_; 
      ...
      put " ";
      put "%nrstr(%macro td_eg7tscg_preprocess;)";
      put " ";
      put "%nrstr(%tu_getdata%(dsetin=ardata.eganal,)";
      put "%nrstr(             dsetout1=ecg1%) )";
      put " ";
      put "%nrstr(proc sort data=ecg1 out=ecg2;)";
      put "%nrstr(  by) &g_centid &g_subjid &g_trtcd &g_trtgrp &keepvar egtestcd egtest;";
      put "%nrstr(run; )";
      put " ";
      put "/* Calculate the maximum change from baseline */";
      put "%nrstr(proc means data=ecg2 max noprint;)";
      put "%nrstr(  output out=ecg3)(drop=_TYPE_ _FREQ_) max=maxchgbl;";
      put "%nrstr(  var stdchgbl;)";
      put "%nrstr(  by) &g_centid &g_subjid &g_trtcd &g_trtgrp &keepvar egtestcd egtest;";
      put "%nrstr(run; )";
      put "%nrstr(quit; )";      
      put " ";           
      put "%nrstr(data final;)";
      put "%nrstr(  set ecg3;)";  
      put "%nrstr(  attrib)";
      put "%nrstr(  maxchgbl length=8 label=%"Maximum change from baseline for x-axis%";)";
      put "%nrstr(  retain) &g_centid &g_subjid &g_trtcd &g_trtgrp &keepvar egtestcd egtest maxchgbl;";
      put "%nrstr(run;)";
      put " ";         
      %if "&dataset"^="" %then %do;
      ...
      put "%nrstr(%mend; )";
      put "%nrstr( )";
      put "%nrstr(%td_eg7tscg_preprocess; )";
      put "%nrstr( )";
    run;    
