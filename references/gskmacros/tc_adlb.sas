/******************************************************************************* 
| Program Name: tc_adlb
|
| Program Version: 1 build 2
|
| HARP Compound/Study/Reporting Effort: 
|
| Program Purpose: To create the ADaM dataset of ADLB domain using the SDTM LB
|                  and supplemental LBSUPP datasets.
|
...
| Change Log 
|
| Modified By:             David Ainsworth (daa62284)
| Date of Modification:    18-Nov-2014
| New version number:      1 build 2
| Modification ID:         DA001
| Reason For Modification: Updated following round 1 of unit testing:
|                          Check for non-missing BNRIND before deriving SHIFT1
|
********************************************************************************/ 
%macro tc_adlb(dsetin=sdtmdata.lb,                     /* Input dataset */
               dsetout=adamdata.adlb,                    /* Output dataset to be created*/
               ...
               datevars=lbdtc,              /* Datetime variables in input dataset to be converted to numeric dates times datetimes*/
               ...
               paramcdmappingdset=diction.adlbparm,        /* Dataset containing PARAMCD mapping for LBTESTCD and qualifiers */
               flaggingsubset  = %STR(PARCAT1 IN ('CHEM','HAEM')), /* IF clause to identify records to be flagged */
...
%end;
  ...
  data &prefix._derive ;
   ...
   %end;
    ...
    %if %tu_chkvarsexist(&lastdset.,&domain.SPEC &domain.TEST &domain.METHOD)=%str() %then 
    %do; 
        PARAM=catx(' ', &domain.SPEC, &domain.TEST, &domain.METHOD, ifc(&domain.STRESU ne '',cats('(',&domain.STRESU,')'),''));;
    %end;
    %else %if %tu_chkvarsexist(&lastdset.,&domain.SPEC &domain.TEST)=%str() %then 
    %do; 
        PARAM=catx(' ', &domain.SPEC, &domain.TEST, ifc(&domain.STRESU ne '',cats('(',&domain.STRESU,')'),''));;
    %end;
    %if %tu_chkvarsexist(&lastdset.,&domain.LOC)=%str() %then 
    %do; 
        PARAM=catx(' ', &domain.LOC, PARAM);
    %end;
    PARAMLBL=catx(' ', propcase(&domain.TEST), ifc(&domain.STRESU ne '',cats('(',&domain.STRESU,')'),''));;
    paramlbl=tranwrd(paramlbl,'Ph ','pH ');
    paramlbl=tranwrd(paramlbl,'Ige','IgE');
 ...
 %let mergevars=%tu_chkvarsexist(&lastdset., &domain.TESTCD &domain.METHOD &domain.SPEC &domain.LOC, Y);
 %let word_cnt=%sysfunc(countw(&mergevars));
  
 /* Ensure only one record per LBTEST, LBSPEC in mapping dataset if LBMETHOD and LBLOC missing, otherwise keep everything */
  data &prefix._parm; 
    set &paramcdmappingdset;
    %if %tu_chkvarsexist(&lastdset., &domain.METHOD &domain.LOC, Y)=%str() %then %do;
       where &domain.METHOD='' and &domain.LOC='';
    %end;
  run;
  ...
  proc sql ;
    ...
    select a.*, b.paramcd
    from &lastdset a left join &prefix._parm b
    ...
    select distinct %do i=1 %to &word_cnt;
                       %scan(&mergevars,&i), 
                    %end;
                    lbtest, lbstresu, paramcd, param, paramlbl
    from &prefix._paramcd
    where lbtestcd ne '' and paramcd='';
  quit;

  data _null_;
    set &prefix._paramchk;
    put "RTWARN" "ING: No PARAMCD for the following rawdata"   lbtestcd= lbspec= lbmethod= paramcd= param= paramlbl=;
  run;
 ...
 /----------------------------------------------------------------------------*/
  ...
  %let lastdset=&prefix._derive1;
   ...
   %if %tu_chkvarsexist(&lastdset.,ANRIND A2INDCD)=%str() %then 
   %do;
     data &prefix._derive2;
       set &lastdset;
       length shift1n 8  shift1 $23;
       /* DA001: Check BNRIND not missing */
       if A2INDCD not in ('P','R') and bnrind ne '' then do;  
  ...
  proc sort data=&lastdset;
      by &g_subjid adtm lbseq PARCAT1 PARAMCD;
  run;
  ...
  /* Derive ANL73FL - flag all records for SUBJID, ADTM (sample), if A1INDCD='H' for ALT and Bilirubin */
  
  %if %tu_chkvarsexist(&lastdset.,A1INDCD)=%str() %then 
  %do;

    data &prefix._alt(keep=&g_subjid adtm);
      set &lastdset(where=(lbtestcd='ALT' and  a1indcd='H'));
      by &g_subjid adtm;
    run;

    data &prefix._bili(keep=&g_subjid adtm);
      set &lastdset(where=(lbtestcd='BILI' and parcat1='CHEM' and a1indcd='H'));
      by &g_subjid adtm;
    run;
    
    data &prefix._anl73fl;
      merge &prefix._alt(in=a) &prefix._bili(in=b); 
      by &g_subjid adtm;
      if a and b then do;
        anl73fl='Y';
        output;
    ...
    data &prefix._derive4;
      merge &lastdset &prefix._anl73fl;
      by &g_subjid adtm;
    run;

    %let lastdset=&prefix._derive4;
