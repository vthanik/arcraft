...
| Date:            30-Jun-2004
|
| Macro Purpose:   POP wrapper macro
|
...
| -----------------  -------------------------------------  -------  ----------
| ENROLDSET          Specifies the ENROL-format SI dataset  REQ      DMDATA.ENROL
|                    which needs to be transformed into a 
|                    POP-format A&R dataset.
|                    Valid values: valid dataset name 
|
| INVESTIGDSET       Specifies the INVESTIG-format SI       OPT      DMDATA.INVESTIG
|                    dataset which will be used along with 
|                    the ENROLDSET to produce a POP-format 
|                    A&R dataset.
|                    Valid values: valid dataset name
|
| DSETOUT            Specifies the name of the output       REQ      ARDATA.POP
...
|                    derivations for this domain code (IE)?
|                    Valid values: Y, N
|
...
|                              to new version of TU macros: demodset, exposuredset, 
|                              racedset, randalldset, randdset, visitdset       
...
%macro tc_pop (
     ...
     enroldset         = DMDATA.ENROL,    /* ENROL dataset name */
     investigdset      = DMDATA.INVESTIG, /* INVESTIG dataset name */
 ...
 %let investigdset      = %nrbquote(&investigdset);
 ...
 %if &enroldset eq %then
 %do;
    %put %str(RTE)RROR: &sysmacroname: The parameter ENROLDSET is required.;
    %let g_abort=1;
 %end;
 ...
 %if %qscan(&enroldset, 1, %str(%()) eq %qscan(&dsetout, 1, %str(%()) %then
 %do;
    %put %str(RTE)RROR: &sysmacroname: The enrol dataset name ENROLDSET(=&enroldset) is the same as the output dataset name DSETOUT(=&dsetout).;
    %let g_abort=1;
 %end;

 %if %qscan(&investigdset, 1, %str(%()) eq %qscan(&dsetout, 1, %str(%()) %then
 %do;
    %put %str(RTE)RROR: &sysmacroname: The investig dataset name INVESTIGDSET(=&investigdset) is the same as the output dataset name DSETOUT(=&dsetout).;
    %let g_abort=1;
 %end;
 ...
 %if &enroldset ne %then
    %if %tu_nobs(&enroldset) lt 0 %then
    %do;
       %put %str(RTE)RROR: &sysmacroname: Data set ENROLDSET(=&enroldset) does not exist.;
       %let g_abort=1;
    %end;

 %if &investigdset ne  %then
    %if %tu_nobs(&investigdset) lt 0 %then
    %do;
        %put %str(RTE)RROR: &sysmacroname: Data set INVESTIGDSET(=&investigdset) does not exist.;
        %let g_abort=1;
    %end;
 ...
 %local prefix studyidvar i;
 %let prefix = _tc_pop;   /* Root name for temporary work datasets */
 ...
 /*
 / Get info from INVESTIG dataset.   
 / YW001: If STUDYID exists on the SI INVESTIG dataset, then merge with ENORL 
 / data set by STUDYID, SUBJID. Else merge by SUBJID                 
 /----------------------------------------------------------------------------*/

 %if &investigdset ne  %then
 %do;
    data &prefix.investig;
       set %unquote(&investigdset);
    run;
    
    %if ( %tu_chkvarsexist(&prefix.investig, STUDYID) eq ) and 
        ( %tu_chkvarsexist(&prefix._ds&i, STUDYID) eq ) %then 
       %let studyidvar=studyid;
    %else 
       %let studyidvar=;     
 
    proc sql noprint;
         create table &prefix._ds%eval(&i+1) as
         select a.*, b.country, b.invname 
         from &prefix._ds&i as a left join &prefix.investig as b
         on a.subjid = b.subjid
         %if %nrbquote(&studyidvar) ne %then %do;
