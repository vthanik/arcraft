...
| Date:            30-Jun-2004
|
| Macro Purpose:   Surgery wrapper macro
|
...
| -----------------  -------------------------------------  -------  ----------
| DSETIN             Specifies the SURGERY-format SI         REQ      DMDATA.SURGERY
|                    dataset which needs to be transformed 
|                    into a SURGERY-format A&R dataset.
|                    Valid values: valid dataset name 
|
| DSETOUT            Specifies the name of the output       REQ      ARDATA.SURGERY
...
|                    Valid values: Y, N
|
| DICTDECODEYN       Dictionary decoding.                   REQ      Y
|
...
|                    Valid values: Y, N
|
| REFDAT             Specify a reference date variable name OPT      SPDT
|                    to pass to %tu_recalcvisit to  
|                    calculate the visit. Will be checked  
|                    in %tu_recalcvisit
|
| REFTIM             Specify a reference time variable name OPT      SPACTTM
|                    to pass to %tu_recalcvisit to  
|                    calculate the visit. Will be checked  
|                    in %tu_recalcvisit
|
...
*******************************************************************************/
%macro tc_surgery (
     dsetin            = DMDATA.SURGERY,  /* Input dataset name */
     dsetout           = ARDATA.SURGERY,  /* Output dataset name */
     ...
     refdat            = spdt,       /* Reference data variable name for recalculating visit and calculating treatment period */
     reftim            = spacttm,    /* Reference data variable name for recalculating visit and calculating treatment period */
 ...
 %local prefix;
 %let prefix = _tc_surgery;   /* Root name for temporary work datasets */
 ...
 %do;
    %tu_decode (
         ...
         decoderename    = spbods=spbodsys spclas=spclass
    );
