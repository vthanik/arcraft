...
| Date:             10DEC2004
|
| Macro Purpose:    A macro to create IDSL standard display DM7
|
...
|                     summarised.
|
| PSFORMAT            Passed to the PROC SUMMARY FORMAT statement. The  raceccd $raceahl.
...
%macro td_dm7(
   ...
   DDDATASETLABEL      =DD dataset for table DM7, /* Label to be applied to the DD dataset */
   ...
   PSFORMAT            =raceccd $raceahl., /* Passed to the PROC SUMMARY FORMAT statement. */
   ...
   /*
   /  Check if &RACECCDVAR exists in &dsetinnumer.
   /-------------------------------------------------------------------------*/                                                                 
   ...
   /*
   / 1. Modify value of &RACECCDVAR to contain only highest level categories of 
   /    race
   / 2. Output subjects in 'Asian' group twice. One assigns &RACECCDVAR and
   ...
   data &l_prefix.final;
      ...
      /* Output sub-group twice */
      if &raceccdvar in ('03A' '03B') then
      do;
         &raceccdvar='03';
         output;
      end; 
