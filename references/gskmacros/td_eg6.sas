...
*  Date:           17sep2003
*
*  Macro purpose:  display macro to generate IDSL eg6 listing
*
...
*                  provided here.                                      %str( varsin=&g_centid &g_subjid,varout=st_eg_cs6, sepc=/)
*  stackvar2       Additional stacked variables (ref: stackvar1)       %str( varsin=age sex race ,varout=st_eg_asr6, sepc=/)     
*  stackvar3       Additional stacked variables (ref: stackvar1)       %str( varsin=&g_ptrtgrp period ,varout=st_eg_pp6, sepc=/) 
*  stackvar4       Additional stacked variables (ref: stackvar1)       %str( varsin=visit egdt, varout=st_eg_vd6, sepc=/)        
*  stackvar5       Additional stacked variables (ref: stackvar1)       %str( varsin=egactdy egperdy, varout=st_eg_ap6)
...
*                  string                                              DD dataset for EG6 listing
...
*                  COLUMN statement syntax                             &g_centid &g_subjid st_eg_cs6 st_eg_asr6 pernum st_eg_pp6 visitnum egdt st_eg_vd6 st_eg_ap6 egintpcd egintp egchbl egfind 
...
*                  that are also defined with COLUMNS                  &g_centid &g_subjid st_eg_cs6 st_eg_asr6 pernum st_eg_pp6 visitnum egdt   
...
*                  rows in the display.                                &g_centid &g_subjid egintpcd pernum visitnum egdt
...
*                  width, width of column header)                      st_eg_cs6 7 st_eg_asr6 10 st_eg_pp6 12 st_eg_vd6 10 st_eg_ap6 6 egintp 12 egchbl 11 egfind 14
...
*                  syntax for a SAS FORMAT statement                   egchbl $yesno.
...
%macro td_eg6(
   ...
   stackvar1      =%str( varsin=&g_centid &g_subjid,varout=st_eg_cs6, sepc=/) , /*  Create stacked variables (e.g., stackvar1=%str(varsin=invid subjid, varout=st_inv_subj,sepc=/,splitc=~))      */
   stackvar2      =%str( varsin=age sex race ,varout=st_eg_asr6, sepc=/), /* Create stacked variables (e.g., stackvar1=%str(varsin=invid subjid, varout=st_inv_subj,sepc=/,splitc=~))       */
   stackvar3      =%str( varsin=&g_ptrtgrp period ,varout=st_eg_pp6, sepc=/), /* Create stacked variables (e.g., stackvar1=%str(varsin=invid subjid, varout=st_inv_subj,sepc=/,splitc=~))       */
   stackvar4      =%str( varsin=visit egdt, varout=st_eg_vd6, sepc=/), /* Create stacked variables (e.g., stackvar1=%str(varsin=invid subjid, varout=st_inv_subj,sepc=/,splitc=~))       */
   stackvar5      =%str( varsin=egactdy egperdy, varout=st_eg_ap6), /* Create stacked variables (e.g., stackvar1=%str(varsin=invid subjid, varout=st_inv_subj,sepc=/,splitc=~))       */
   ...
   dddatasetlabel =DD dataset for EG6 listing, /* Label to be applied to the DD dataset  */
   ...
   columns        =&g_centid &g_subjid st_eg_cs6 st_eg_asr6 pernum st_eg_pp6 visitnum egdt st_eg_vd6 st_eg_ap6 egintpcd egintp egchbl egfind, /* Columns to be included in the listing (plus spanned headers)  */
   ordervars      =&g_centid &g_subjid st_eg_cs6 st_eg_asr6 pernum st_eg_pp6 visitnum egdt, /*  Order variables  */
   ...
   noprintvars    =&g_centid &g_subjid egintpcd pernum visitnum egdt, /* No print variables, used to order the display */
   byvars         =,          /* By variables */
   flowvars       =_all_,     /* Variables with flow option */
   widths         =,          /* Column widths */
   defaultwidths  =st_eg_cs6 7 st_eg_asr6 10 st_eg_pp6 12 st_eg_vd6 10 st_eg_ap6 6 egintp 12 egchbl 11 egfind 14, /* List of default column widths */
   ...
   formats        =egchbl $yesno., /* Format specification (valid SAS syntax) */
   ...
   %local l_prefix l_workdata l_vlen;
   %let l_prefix=_eg6;
   %let l_workdata=&dsetin;
   
   /*
   / If EGFIND, EGFINDCD and EGFOTH are in &DSETIN and EGFINDCD equals 'E99', 
   / set EGFIND to 'Other: ' + EGFOTH 
   /-----------------------------------------------------------------------*/
   ...
   %if %nrbquote(&dsetin) NE %then %do;            
      ...
      %if %tu_nobs(&dsetin) GT 0 %then %do;
          ...
          %if %nrquote(%tu_chkvarsexist(&dsetin, egfind egfindcd egfoth)) EQ %then %do;
          
             data _NULL_;            
                if 0 then set &l_workdata ;
                call symput('l_vlen', compress(max(vlength(egfind), vlength(egfoth) + 8)));
             run;
             
             data &l_prefix.eg6;
                length egfind $&l_vlen ;
                set &dsetin;
                if upcase(egfindcd) eq 'E99' then
                   egfind="Other: "||left(egfoth);
             run;   
             
             %let l_workdata=&l_prefix.eg6;
                
          %end; /* end-if on variables exist */       
            
      %end; /* end-if on &DSETIN is not empty */ 
      
   %end; /* end-if on dsetin is not blank */
