...
| Date:            29-Jun-2004
|
| Macro Purpose:   Exposure wrapper macro
|
...
| -----------------  -------------------------------------  -------  ----------
| DSETIN             Specifies the EXPOSURE-format SI       REQ      DMDATA.EXPOSURE
|                    dataset which needs to be transformed 
|                    into an EXPOSURE-format A&R dataset.
|                    Valid values: valid dataset name 
|
| DSETOUT            Specifies the name of the output       REQ      ARDATA.EXPOSURE
|                    dataset to be created.
|                    Valid values: valid dataset name 
|
| RANDDSET           Specifies the name of an SI            REQ      DMDATA.RAND
|                    RAND-formatted dataset. 
|                    Valid values: valid dataset name 
|
...
|                    derivations for this domain code (EX)?
|                    Valid values: Y, N
|
...
|
| DURATIONUNITS      Units to be used for exposure          OPT      Days
|                    duration.
|                    Valid values: Days or Hours. 
|
...
| Date of Modification:     19-Oct-05
| New version/draft number: 2/1
| Modification ID:          YW002
| Reason For Modification:  1. Added new parameter XOVARSFORPGYN and RECALCVISITYN.
|                              They are validated 
|                           2. Added new parameter REFDAT and REFTIM. They are 
|                              passed to %tu_recalcvisit and %tu_calctpernum
|                           3. Added call of %tu_calctpernum
|                           4. Added call of %tu_recalvisit
|
| Modified By:              Yongwei Wang
...
|                              investigdset, racedset, randalldset, 
...
| Date of Modification:     15-May-08
| New version/draft number: 3/2
| Modification ID:          BA001
| Reason For Modification:  Added a check for missing RANDDSET parameter.
|
| Modified By:
...
*******************************************************************************/
%macro tc_exposure (
     dsetin            = DMDATA.EXPOSURE,  /* Input dataset name */
     dsetout           = ARDATA.EXPOSURE,  /* Output dataset name */
     ...
     recalcvisityn     = N,       /* Recalculate visit based on the CONMEDS start date */
     ...
     refdat            = exstdt,  /* Reference data variable name for recalculating visit and calculating treatment period */
     reftim            = exsttm,  /* Reference data variable name for recalculating visit and calculating treatment period */
     ...
     durationunits     = Days     /* Units to be used for exposure duration */
        );
 ...
 %if &durationunits eq %then
 %do;
    %put %str(RTE)RROR: &sysmacroname: The parameter DURATIONUNITS is required.;
    %let g_abort=1;
 %end;
 ...
 %local prefix exposuredset;
 %let prefix = _tc_exposure;   /* Root name for temporary work datasets */
 ...
 %do;
    %tu_decode (
         ...
         decoderename    = doseu=doseunit dosefm=doseform dosefr=dosefreq
    );
