...
| Date:               20Aug2007
|
| Macro purpose:      The unit will produce a listing of listing of information on liver biopsy results
|
...
|                     Valid values: one or more variable names     st_all_asr lpdt
|                     from DSETIN plus other elements of valid     st_liver_dsd7
|                     PROC REPORT COLUMN statement syntax, but     biopsz lptestcd
|                     not including report_item=alias syntax       lptest lporrscd
|                                                                  lporres
|
...
|                     dataset                                      new  listing
|                     Valid values: a non-blank text string        LIVER7
|
| DEFAULTWIDTHS       This is a list of default widths for ALL     st_all_cs 7
|                     columns and will usually be defined by the   st_all_asr 20
|                     DD macro.  This parameter specifies column   st_liver_dsd7 10
|                     widths for all variables not listed in the   biopsz 7 lptest
|                     WIDTHS parameter.                            20 lporres 30
...
|                     ORDERVARS
|
| DSETIN              Specifies the name of the input dataset      ardata.lbiopsy
|                     Valid values: name of an existing dataset,
|                     pre-sorted by BYVARS
|
| FLOWVARS            Variables to be defined with the flow        st_all_cs
|                     option.                                      st_all_asr
|                     Valid values: one or more variable names     st_liver_dsd7
|                     from DSETIN that are also defined with       lptest lporres
|                     COLUMNS
...
|                     that are given the PROC REPORT define        &g_subjid lpdt
|                     statement attribute noprint.                 lptestcd lporrscd
|                     Valid values: one or more variable names
|                     from DSETIN that are also defined with
|                     COLUMNS
...
|                     ORDER                                        st_all_cs
|                     Valid values: one or more variable names     st_all_asr lpdt
|                     from DSETIN that are also defined with       st_liver_dsd7
|                     COLUMNS                                      biopsz lptestcd
|                                                                  lptest lporrscd
|
...
|                                                                  sepc=/)
|
| STACKVAR3           Same as STACKVAR1                            %str(varsin=lpdt
|                                                                  actdy,
|                                                                  varout=st_liver_ds
|                                                                  d7, sepc=/)
|
...
%macro td_liver7(
   ...
   COLUMNS             =&g_centid &g_subjid st_all_cs st_all_asr lpdt st_liver_dsd7 biopsz lptestcd lptest lporrscd lporres, /* Column parameter */
   ...
   DDDATASETLABEL      =DD dataset for new  listing LIVER7, /* Label to be applied to the DD dataset */
   DEFAULTWIDTHS       =st_all_cs 7 st_all_asr 20 st_liver_dsd7 10 biopsz 7 lptest 20 lporres 30, /* List of default column widths */
   DESCENDING          =,                  /* Descending ORDERVARS */
   DSETIN              =ardata.lbiopsy,    /* Input dataset */
   FLOWVARS            =st_all_cs st_all_asr st_liver_dsd7 lptest lporres, /* Variables with flow option */
   ...
   NOPRINTVARS         =&g_centid &g_subjid lpdt lptestcd lporrscd, /* No print vars (usually used to order the display) */
   ...
   ORDERVARS           =&g_centid &g_subjid st_all_cs st_all_asr lpdt st_liver_dsd7 biopsz lptestcd lptest lporrscd, /* Order variables */
   ...
   SKIPVARS            =lptest,            /* Break after <var> / skip; */
   ...
   STACKVAR3           =%str(varsin=lpdt actdy, varout=st_liver_dsd7, sepc=/), /* Same as STACKVAR1 */
   ...
   %local l_prefix;
   %let l_prefix=_liver7;

   /* Combine LPORRSSP with LPRRRES */

   %if %tu_nobs(&dsetin) gt 0 %then
   %do;
      %if %tu_chkvarsexist(&dsetin, LPORRSCD LPORRES LPORRSSP) eq %then
      %do;
         data &l_prefix.dsetin;
            length LPORRES $300;
            set &dsetin;
            if substr(left(LPORRSCD), 2, 2) eq '99' then LPORRES='Other: ' || LPORRSSP;
         run;
