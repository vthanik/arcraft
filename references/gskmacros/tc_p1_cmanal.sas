...
| Date:            15th May 2008
|
| Macro Purpose:   Create A&R CMANAL dataset (ARDATA.CMANAL)
|                  (PG & XO studies)
|
...
|                 DOCUMENTATION FOR TC_P1_CMANAL
| ------------------------------------------------------------------
| ------------------------------------------------------------------
|
...
| Modification ID:          n/a
| Reason For Modification:  Changes 1-3 are based on the updated to TC_CMANAL macro
...
%macro tc_p1_cmanal (
     ...
     dsetin_cmanal       = DMDATA.CONMEDS,      /* Input  dataset name */
     dsetout             = ARDATA.CMANAL,       /* Output dataset name */
  ...
  %let dsetin_cmanal       = %nrbquote(&dsetin_cmanal.);
  %let dsetout             = %nrbquote(&dsetout.);
  %let preprocess          = %nrbquote(&preprocess.);
  %let postprocess         = %nrbquote(&postprocess.);
  %let prohibitmed_subset  = %nrbquote(&prohibitmed_subset.);
  ...
  %let listvars=DSETIN_CMANAL DSETOUT;
  ...
  /----------------------------------------------------------------------------*/
  %if %qscan(&dsetin_cmanal, 1, %str(%()) eq %qscan(&dsetout, 1, %str(%()) %then
  %do;
    %put %str(RTE)RROR: &sysmacroname: The input dataset name DSETIN(=&dsetin_cmanal) is the same as output data set name DSETOUT(=&dsetout).;
    %let pv_abort=1;
  %end;
  ...
  /----------------------------------------------------------------------------*/
  * DMDATA.CMANAL exists? ;
  %if not %sysfunc(exist(&dsetin_cmanal)) %then 
  %do;
    %put RTE%str(RROR): &sysmacroname.: &dsetin_cmanal does not exist;
    %let pv_abort = 1;
  %end;
  ...
  /----------------------------------------------------------------------------*/
  %local prefix preprocessdset cmdset postprocessdset;
  %let prefix = _tc_p1_cmanal ;   /* Root name for temporary work datasets */
  ...
  /----------------------------------------------------------------------------*/
  %let preprocessdset = &dsetin_cmanal ;
  ...
  /*
  / Call DAMA macro tc_cmanal
  /----------------------------------------------------------------------------*/
  %tc_cmanal  (dsetin            = &preprocessdset    /* Input dataset name */
  ...
  * Set a temporary macro variable containing dataset reference ;
  ...
  %IF &G_STYPE = XO OR (&G_STYPE = PG AND &XOVARSFORPGYN = Y) %THEN %DO;
    ...
    * Set a temporary macro variable containing dataset reference ;
    %let cmdset = &prefix._cm2;

  %END; /*EndOf IF &G_STYPE = XO OR (&G_STYPE = PG AND &XOVARSFORPGYN = Y) */
  ...
  %do;
    ...
    /---------------------------------------------------------------------------*/
    %tu_rantrt ( dsetin      = &cmdset          /* Input dataset name */
  ...
  %do;
    ...
    /---------------------------------------------------------------------------*/
    %tu_p1_periodday ( dsetin         =&cmdset           /* Input dataset                                    */
    ...
    * Set a temporary macro variable containing dataset reference ;
    %let cmdset = &prefix._cm6;

  %end; /*EndOf if g_stype = XO */

  /*
  / Code to flag prohibited medications
  /   This step allows the user to supply simple SAS code to create a flagging
  /   variable for prohibited medications.
  /---------------------------------------------------------------------------------*/
  %if %nrbquote(&prohibitmed_subset) ne %then %do;
     data &prefix.prohibitmed_subset;
        set &cmdset;
        %unquote(&prohibitmed_subset);;
     run;
     %let postprocessdset=&prefix.prohibitmed_subset;
  %end;
  %else %do;
    %let postprocessdset = &cmdset;
  %end;
