...
| Date:            27-Jun-2004
|
| Macro Purpose:   Vitals wrapper macro
|
...
| -----------------  -------------------------------------  -------  ----------
| DSETIN             Specifies the VITALS-format SI         REQ      DMDATA.VITALS
|                    dataset which needs to be transformed 
|                    into a VITALS-format A&R dataset.
|                    Valid values: valid dataset name
|
| DSETOUT            Specifies the name of the output       REQ      ARDATA.VITALS
...
|                    Valid values: Y, N
|
| DERIVATIONYN       Call %tu_derive to perform             REQ      Y
|                    specific derivations for this 
|                    domain code (VS)?
|                    Valid values: Y, N
|
| ATTRIBUTESYN       Call %tu_attrib to assign the          REQ      Y
|                    A&R-defined attributes to the 
|                    output dataset?
|                    Valid values: Y, N
|
...
|                    Valid values: Y, N
|
| REFDAT             Specify a reference date variable name OPT      VSDT
|                    to pass to %tu_calctpernum to  
|                    calculate treatment period. Will be   
|                    checked in %tu_calctpernum 
|
| REFTIM             Specify a reference time variable name OPT      VSACTTM
|                    to pass to %tu_calctpernum to  
|                    calculate treatment period. Will be   
|                    checked in %tu_calctpernum
|
...
| Reason For Modification:  1. Added new parameter XOVARSFORPGYN and REFDAT, 
|                              REFTIM  
|                           2. Added call of %tu_calctpernum
|                           3. Passed XOVARSFORPGYN to %tu_derive
|                           4. Changed calling order of TU macros
|
| Modified By:              Yongwei Wang
...
%macro tc_vitals (
 ...
 %if &timeslicingyn eq N and &treatvarsyn eq Y %then
 %do;
    %put %str(RTW)ARNING: &sysmacroname: TIMESLICINGYN is N and TREATVARSYN is Y. Timeslicing variables will not be added;
    %put %str(RTW)ARNING: &sysmacroname: to the dataset, which may prevent treatment variables from being added.;
 %end;
 ...
 %local prefix;
 %let prefix = _tc_vitals;   /* Root name for temporary work datasets */
 ...
 /*
 / If &XOVARSFORPGYN equals Y or &G_STYPE equals XO, 
 ...
 %if (&xovarsforpgyn eq Y) and (%nrbquote(&reftim) ne) %then
 %do;
