/* Used by the HARP application to insert define data into database for processing */
/* Used from version 8 forward */
%macro ta_imp_define2_type(
);
...
/*   ENV [DEV|TST|VAL|PRD]                     */
/*   RUNID - Used to save to the database      */
...
%GLOBAL pArenv pEnv pVer user password path; 
...
/* Get the standard version from the define */
%let xmlfilename = "/&pArenv/define_stage/working/&pREID/sourcexml/define.xml" ;
%let mapfilename = "/&pArenv/define_stage/working/&pREID/referencexml/definestudy.map";
filename xmlfile &xmlfilename;
filename mapfile &mapfilename;

%put &xmlfilename;
%put &mapfilename;

libname xmlfile xmlv2 xmlmap=mapfile automap=replace prefixattributes=no;

data x;
  set xmlfile.metadataversion;

run;
  
data _null_;
   set x;
   call symput('pVer' ,STRIP(standardversion));
run;

%put new standardversion = &pVer;

libname oralib oracle user=&user password=&password path="&path";

**********************************************************************************;

* create_sourcemetadata_fromsasdefine.sas                                        *;
*                                                                                *;

* This sample driver module performs the setup steps to derive source metadata   *;
* files for a CDISC study from the SAS representation of a Define-XML v2 file.   *;
*                                                                                *;
...
%GLOBAL _cstTrgStandard _cstTrgStandardVersion;
...
%let _cstStandardSubFolder=%lowcase(&_cstTrgStandard);
...
proc sql;
  ...
  values ("&_cstStandard"    "&_cstStandardVersion"     "sourcedata"        ""           "srcdata"  "libref"  "input"  "folder"   "N"  "" "&studyRootPath/deriveddata/&_cstStandardSubFolder"                    . ""                           "")
...
proc sql;
  insert into oralib.harp_define_import_tables
  select 
   &pRUNIN AS IMPORT_ID,
  table as xtable,
  label, 
  ...
  standard,
  standardversion 
  from trgmeta.source_tables;

  insert into oralib.harp_define_import_study
    select 
         &pRUNIN AS IMPORT_ID, 
         &pREID AS RE_ID,
         DATETIME() format = datetime20. AS import_time,
         studyname,
	 studydescription,
         protocolname,
         formalstandardname,
         ...
         standard,
         standardversion
 from trgmeta.source_study;

  insert into oralib.harp_define_import_columns
    select 
         &pRUNIN AS IMPORT_ID, 
         table           as xtable,
         column          as xcolumn,
         ...
         standard,
         standardversion
 from trgmeta.source_columns;


 insert into oralib.harp_define_import_codelists
    select 
         &pRUNIN AS IMPORT_ID, 
         codelist,
         codelistname,                      
         ...
         standard,
         standardversion
 from trgmeta.source_codelists;

 insert into oralib.harp_define_import_documents
    select 
         &pRUNIN AS IMPORT_ID, 
         doctype,
         href,
         ...
         standard,
         standardversion
 from trgmeta.source_documents;


 insert into oralib.harp_define_import_values
    select 
         &pRUNIN AS IMPORT_ID, 
         table            as xtable,
         column           as xcolumn,
         ...
         standard,
         standardversion
 from trgmeta.source_values;

  insert into oralib.harp_define_import_ar
    select 
         &pRUNIN AS IMPORT_ID, 
         displayidentifier,
         displayname,
         ...
         standard,
         standardversion
 from trgmeta.source_analysisresults;
