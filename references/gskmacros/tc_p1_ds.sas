...
| Date:            26 June 2008
|
| Macro Purpose:   Create DS A&R data set for Phase1 requirements
|                  (PG & XO studies)
|
...
%macro tc_p1_ds (
  dsetin_ds           = dmdata.ds,                               /* Input dataset name */
  dsetout             = ardata.ds,                               /* Output dataset name */
  preprocess          = if dsfail='N' and dsstdt=. then delete;, /* Any processing required after reading in the input dataset */
  postprocess         = ,                                        /* Any processing required before writing out to final output dataset */
  addmultiobsyn       = N,                                       /* Append additional rows to output dataset (1 row per subject and period until subject withdrew from study) */
  ...
  refdat              = dsstdt,                                  /* Reference data variable name for recalculating visit */
  ...
  /----------------------------------------------------------------------------*/
  %let dsetin_ds           = %nrbquote(&dsetin_ds.);               
  ...
  %let pv_abort = 0 ;
  %let prefix = _tc_p1_ds;   /* Root name for temporary work datasets */

  /*
  / If the input dataset name is the same as the output dataset name,
  / write an error to the log.
  /----------------------------------------------------------------------------*/
  %if %qscan(&dsetin_ds, 1, %str(%()) eq %qscan(&dsetout, 1, %str(%()) %then
    %do;
      %put %str(RTE)RROR: &sysmacroname: The input dataset name DSETIN(=&dsetin_ds) is the same as output data set name DSETOUT(=&dsetout).; 
      %let pv_abort=1;
    %end;
  ...
  /----------------------------------------------------------------------------*/
  %let listvars=DSETIN_DS DSETOUT;
  ...
  /* Macro Parameter DSETIN_DS */
  %if not %sysfunc(exist(&dsetin_ds)) %then 
    %do;
      %put RTE%str(RROR): &sysmacroname.: DSETIN_DS(=&dsetin_ds) does not exist;
      %let pv_abort = 1;
    %end;
  ...
  /*
  / Call DAMA macro tc_ds
  /----------------------------------------------------------------------------*/
  %tc_ds (
   dsetin              = &preprocessdset,      /* Input dataset name */
   dsetout             = &prefix._ds1,        /* Output dataset name */
  ...
  %let postprocessdset = &prefix._ds1;
  ...
  %if &g_stype = XO or (&g_stype = PG and &xovarsforpgyn = Y) %then %do;
    ...
    %if "&calctpernum_version"="TU_P1_CALCTPERNUM" %then %do;
      ...
      /* Set a temporary macro variable containing dataset reference */
      %let postprocessdset = &prefix._ds2;
      ...
      %tu_p1_calctpernum ( dsetin       = &postprocessdset  /* Input dataset name */ 
                          ,dsetout      = &prefix._ds3      /* Output dataset name */
                          ,datadomain   = DS2               /* Data Domain - One of (AE,BL,CM,DS,DS2,SD,IP) */
    ...
    /* Set a temporary macro variable containing dataset reference */
    %let postprocessdset = &prefix._ds3;
  ...
  / (1 row per subject and period until subject withdrew from study) 
  /-----------------------------------------------------------------*/
  %if &g_stype=XO and &addmultiobsyn=Y %then %do;

    data &prefix._ds4 (drop=tperiod pernum);
      set &postprocessdset;
  ...
  %if &treatvarsyn=Y %then %do;
    ...
    %tu_rantrt ( dsetin      = &postprocessdset /* Input dataset name */
                ,dsetout     = &prefix._ds6     /* Output dataset name */
                ,ptrtcdinf   = &ptrtcdinf       /* Informat to derive PTRTCD from PTRTGRP */ 
    ...
    %if &g_stype=PG %then %do;
      ...
      %tu_p1_acttrt_pg (dsetin         = &prefix._ds6,     /* Name of input dataset */
                        dsetout        = &prefix._ds7,     /* Name of output dataset */ 
    ...
    %else %if &g_stype=XO %then %do;
      ...
      %tu_p1_acttrt_xo (dsetin         = &prefix._ds6,    /* Name of input dataset */
                        dsetout        = &prefix._ds7,    /* Name of output dataset */ 
                        trt_dev_exists = &trt_dev_exists, /* Do treatment deviations exist for your study? */
                        exposuredset   = &exposuredset,   /* Name of EXPOSURE dataset to use */
                        tmslicedset    = &tmslicedset     /* Name of TMSLICE dataset to use */
    ...
    /*
    / Set the Treatment at the time of withdrawal to the actual treatment last taken
    /--------------------------------------------------------------------------------*/
    data &prefix._ds8;
      set &prefix._ds7;
        %if &g_stype = PG %then %str(dsacttrt=atrtgrp;);
        %else %if &g_stype = XO %then %str(dsacttrt=tpatrtgp;);
    run;

    /* Set a temporary macro variable containing dataset reference */
    %let postprocessdset = &prefix._ds8;
  ...
  %if &g_stype=XO %then %do;
    ...
    %tu_p1_periodday ( dsetin         = &postprocessdset /* Input dataset                                    */
                      ,dsetout        = &prefix._ds9     /* Output dataset                                   */
    ...
    /* Set a temporary macro variable containing dataset reference */
    %let postprocessdset = &prefix._ds9;
