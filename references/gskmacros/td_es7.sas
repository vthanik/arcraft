...
| Date:               01Jun2007
|
| Macro purpose:      display macro to generate IDSL ES7 listing
|
...
|                     specification.  Including spanning titles    &g_subjid dsstdt
|                     and variable names                           dssbrscd dssbrs
|                     Valid values: one or more variable names
...
|
| DEFAULTWIDTHS       This is a list of default widths for ALL     &g_centid 8
|                     columns and will usually be defined by the   &g_subjid 8
|                     DD macro.  This parameter specifies column   dsstdt 15 dssbrs
|                     widths for all variables not listed in the   50
|                     WIDTHS parameter.
...
|                     ORDERVARS
|
| DSETIN              Specifies the name of the input dataset      ardata.ds(where
|                     Valid values: name of an existing dataset,   =(dsscatcd in 
|                     pre-sorted by BYVARS                         (2)))
|
| FLOWVARS            Variables to be defined with the flow        dssbrs
|                     option.
|                     Valid values: one or more variable names
|                     from DSETIN that are also defined with
|                     COLUMNS
...
|                     COLUMNS
|
| LABELS              Variables and their label for display. For  dsstdt='Date of 
|                     use where label for display differs to the  Screen Failure'
|                     label on the DSETIN                         dssbrs='Reason'
...
|
| NOPRINTVARS         Variables listed in the COLUMN parameter     dssbrscd
...
|                     PROC REPORT define statement attribute       &g_subjid dsstdt
|                     ORDER                                        dssbrscd
|                     Valid values: one or more variable names
|                     from DSETIN that are also defined with
|                     COLUMNS
...
|
| STACKVAR1-STACKVAR  Specifies parameters to pass to              (Blank)
| 15                  %tu_stackvar in order to stack variables
|                     together.  See Unit Specification for HARP
|                     Reporting Tools %TU_STACKVAR[4] for more
...
| Modification ID:         n/a
| Reason for modification: Changed label of DSSBRS to 'Reason' in default values of
|                          LABELS, based on UAT results.
|-----------------------------------------------------------------------------------------
| Change Log
| Modified by:
...
%macro td_es7(
   ...
   COLUMNS             =&g_centid &g_subjid dsstdt dssbrscd dssbrs, /* Column parameter */
   ...
   DDDATASETLABEL      =DD dataset for ES7 listing, /* Label to be applied to the DD dataset */
   DEFAULTWIDTHS       =&g_centid 8 &g_subjid 8 dsstdt 15 dssbrs 50, /* List of default column widths */
   DESCENDING          =,                  /* Descending ORDERVARS */
   DSETIN              =ardata.ds(where=(dsscatcd in (2))), /* Input dataset */
   FLOWVARS            =dssbrs,            /* Variables with flow option */
   FORMATS             =,                  /* Format specification */
   IDVARS              =,                  /* Variables to appear on each page should the report be wider than 1 page */
   LABELS              =dsstdt='Date of Screen Failure' dssbrs='Reason',/* Label definitions. */
   LEFTVARS            =dsstdt,            /* Left justify variables */
   LINEVARS            =,                  /* Order variable printed with line statements. */
   NOPRINTVARS         =dssbrscd,          /* No print vars (usually used to order the display) */
   ...
   ORDERVARS           =&g_centid &g_subjid dsstdt dssbrscd, /* Order variables */
   ...
   /* Combine DSSBRSSP with DSSBRS */

   %if %tu_nobs(&dsetin) ge 0 %then
   %do;
