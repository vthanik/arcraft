...
| Date:            29-Jun-2004
|
| Macro Purpose:   Medhist events wrapper macro
|
...
| -----------------  -------------------------------------  -------  ----------
| DSETIN             Specifies the MEDHIST-format SI        REQ      DMDATA.MEDHIST
|                    dataset which needs to be transformed 
|                    into a MEDHIST-format A&R dataset.
|                    Valid values: valid dataset name
|
| DSETOUT            Specifies the name of the output       REQ      ARDATA.MEDHIST
...
|                    derivations for this domain code (MH)?
|                    Valid values: Y, N
|
...
*******************************************************************************/
%macro tc_medhist (
     dsetin            = DMDATA.MEDHIST,  /* Input dataset name */
     dsetout           = ARDATA.MEDHIST,  /* Output dataset name */
 ...
 %local prefix;
 %let prefix = _tc_medhist;   /* Root name for temporary work datasets */
 ...
 %do;
    %tu_decode (
         ...
         decoderename    = mhbods=mhbodsys mhclas=mhclass
    );
