...
| -----------------------------------------------------------------------------------
| DSETIN              Specifies the LAB-format SI dataset which    dmdata.ds
|                     needs to be transformed into a LAB-format
|                     A&R dataset.
|                     Valid values: valid dataset name
|
| DSETOUT             Specifies the name of the output dataset to  ardata.ds
|                     be created.
|                     Valid values: valid dataset name
|
...
|                     Valid values: Y, N.
|
| REFDAT              Specify a reference date variable name to    dsstdt
...
|
| DECODERENAME        DECODERENAME      By default, a coded        (Blank)
...
| Date of Modification:     31-Jan-08
...
%macro tc_ds (
   DSETIN              =dmdata.ds,         /* Input dataset name */
   ...
   %let prefix = _tc_ds;   /* Root name for temporary work datasets */
   %let demodset=dmdata.demo; /* should be added to parameter when DMDATA.DEMO is in parameter value of all utility macros */
  ...
  %if %nrbquote(&dsetin) ne %then
   ...
   /*
   / Recalculate visit based on date of disposit.
   /----------------------------------------------------------------------------*/
