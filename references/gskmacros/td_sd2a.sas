...
| Date:               01Jun2007
|
| Macro purpose:      Display macro to generate new IDSL listing SD2
|
...
|
| COLUMNS             A PROC REPORT column statement               &g_trtcd
|                     specification.  Including spanning titles    &g_trtgrp
|                     and variable names                           &g_centid
|                     Valid values: one or more variable names     &g_subjid actdt
|                     from DSETIN plus other elements of valid     st_all_cs actdy 
|                     PROC REPORT COLUMN statement syntax, but     sdrscd sdrs 
|                     not including report_item=alias syntax       sdsubrcd sdsubr
|
...
|                     dataset                                      new ES2 listing
|                     Valid values: a non-blank text string
|
| DEFAULTWIDTHS       This is a list of default widths for ALL     &g_trtgrp 15
|                     columns and will usually be defined by the   st_all_cs 10
|                     DD macro.  This parameter specifies column   actdt 10 actdy 5     
|                     widths for all variables not listed in the   sdrs 25 sdsubr 25   
|                     WIDTHS parameter.                            
...
|                     ORDERVARS
|
| DSETIN              Specifies the name of the input dataset      ardata.ipdisc
|                     Valid values: name of an existing dataset,
|                     pre-sorted by BYVARS
|
| FLOWVARS            Variables to be defined with the flow        &g_trtgrp  
|                     option.                                      st_all_cs
|                     Valid values: one or more variable names     sdrs sdsubr
|                     from DSETIN that are also defined with
|                     COLUMNS
...
|
| NOPRINTVARS         Variables listed in the COLUMN parameter     &g_trtcd   
|                     that are given the PROC REPORT define        &g_trtgrp 
|                     statement attribute noprint.                 &g_centid 
|                     Valid values: one or more variable names     &g_subjid 
|                     from DSETIN that are also defined with       sdrscd
|                     COLUMNS                                      sdsubrcd
...
|                     define attribute order=internal
|
| ORDERVARS           List of variables that will receive the      &g_trtcd
|                     PROC REPORT define statement attribute       &g_trtgrp
|                     ORDER                                        &g_centid &g_subjid 
|                     Valid values: one or more variable names     st_all_cs
|                     from DSETIN that are also defined with       actdt actdy 
|                     COLUMNS                                      sdrscd sdrs 
|                                                                  sdsubrcd
|
...
|
| STACKVAR1-STACKVAR  Specifies parameters to pass to              %str(varsin=&g_centid 
| 15                  %tu_stackvar in order to stack variables     &g_subjid, 
...
|
| STACKVAR2-STACKVAR  Same as STACKVAR1                            (Blank)
| 15                  
|
...
%macro td_sd2a(
   ...
   COLUMNS             =&g_trtcd &g_trtgrp &g_centid &g_subjid st_all_cs actdt actdy sdrscd sdrs sdsubrcd sdsubr, /* Column parameter */
   ...
   DDDATASETLABEL      =DD dataset for new SD2 listing, /* Label to be applied to the DD dataset */
   DEFAULTWIDTHS       =&g_trtgrp 15 st_all_cs 10 actdt 10 actdy 5 sdrs 25 sdsubr 25, /* List of default column widths */
   DESCENDING          =,                  /* Descending ORDERVARS */
   DSETIN              =ardata.ipdisc,         /* Input dataset */
   FLOWVARS            =&g_trtgrp st_all_cs sdrs sdsubr, /* Variables with flow option */
   ...
   NOPRINTVARS         =&g_trtcd &g_centid &g_subjid sdrscd sdsubrcd, /* No print vars (usually used to order the display) */
   ...
   ORDERVARS           =&g_trtcd &g_trtgrp &g_centid &g_subjid st_all_cs actdt actdy sdrscd sdrs sdsubrcd, /* Order variables */
   ...
   %local l_prefix;
   %let l_prefix=_tdsd2a;

   /* Combine sdsubrSP with sdsubr and DSSBSP with sdrs*/

   %if %tu_nobs(&dsetin) gt 0 %then
   %do;
      %if %tu_chkvarsexist(&dsetin, sdsubrcd sdsubr sdsubrsp) eq %then
      %do;
         data &l_prefix.dsetin;
            length sdsubr $300;
            set &dsetin;
            if (upcase(sdsubrcd)) eq 'OT' then sdsubr='Other: ' || sdsubrsp ;
            else if not missing(sdsubrsp) then sdsubr=trim(sdsubr)||': '||sdsubrsp;
         run;

         %let dsetin=&l_prefix.dsetin;
      %end; /* %tu_chkvarsexist(&dsetin, sdsbrscd sdsbrs sdsbrssp) eq */

      %if %tu_chkvarsexist(&dsetin, sdrs sdrssp) eq %then
      %do;
         data &l_prefix.dsetin;
            length sdrs $300;
            set &dsetin;
            if not missing(sdrssp) then sdrs=trim(sdrs)||': '||sdrssp;
         run;

         %let dsetin=&l_prefix.dsetin;
      %end; /* %tu_chkvarsexist(&dsetin, sdrs sdrssp) eq */
