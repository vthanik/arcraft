...
|                    Table (PKPT4P) as defined in the ISDL Standard Data Displays document.
...
|
| COMPLETETYPESVARS        Passed as %tu_sumstatsinrows parameter of the same name.  &g_trtcd &g_trtgrp,   
|
...
|
| DDDATASETLABEL           Passed as %tu_sumstatsinrows parameter of the same name.  DD Dataset for Table PKPT4P (Req)
|
| DEFAULTWIDTHS            Passed as %tu_sumstatsinrows parameter of the same name.  pppar 12 tt_svnm 15 visit 10 tt_p: 20
|
...
|    
| STATSDPS                 Passed as %tu_sumstatsinrows parameter of the same name.  geomeanclm +1 meanclm +1 std +2 stdlog +2 median +1 (Req)
|
...
%MACRO td_pkpt4p(
   ...
   DDDATASETLABEL           =DD dataset for PKPT4P, /* Label to be applied to the DD dataset */
   DEFAULTWIDTHS            =pppar 12 tt_svnm 15 visit 10 tt_p: 20, /* List of default column widths */
   ...
   NOPRINTVARS              =tt_svid visitnum,     /* No print variables (usually used to order the display) */
