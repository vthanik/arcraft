...
| Date:            May 10, 2011
|
| Macro Purpose:   Create LB10 display using TIBCO Spotfire Clinical Graphics (TSCG)
|
...
|                    Valid values: Y, N
|
| NRTYPE             =NR if Upper limit of Normal Range will be used.     NR
...
|
************************************************************************************************/
%macro td_lb10tscg(InputUsage        = ,                   /* Style of input data: C=Create pre-processing program, D=Use existing dataset as input for template, P=Call pre-processing program, U=Use template */
  ...
  * Parameter cleanup;
  ...
  %macro splusPlot_write_template(fname);
      ...
      data _null_; 
        ...
        put " ";
        put "%nrstr(%macro td_lb10tscg_preprocess;)";
        put " ";
        put "%nrstr(%tu_getdata%(dsetin=ardata.lab,)";
        put "%nrstr(            dsetout1=_lb10tscg_1%) )";
        put " ";
        put "%nrstr(data _lb10tscg_2 %(keep=) &g_centid &g_subjid &keepvar &g_trtcd &g_trtgrp %nrstr(lbtestcd lbtest lbstunit basevar normlvar) ); ";
        put "%nrstr(  set _lb10tscg_1; )";
        ...
        put "%nrstr(run; )";
        put "%nrstr( )";
        put "%nrstr(proc sort data=_lb10tscg_2 out=_lb10tscg_2;)";
        put "%nrstr(  by) &g_centid &g_subjid &keepvar &g_trtcd &g_trtgrp %nrstr(lbtestcd lbtest lbstunit normlvar; )";
        put "%nrstr(run; )";
        put "%nrstr( )";
        put "%nrstr(data _lb10tscg_3;)";
        put "%nrstr(  set _lb10tscg_2; )";
        put "%nrstr(  by) &g_centid &g_subjid &keepvar &g_trtcd &g_trtgrp %nrstr(lbtestcd lbtest lbstunit normlvar; )";
        put "%nrstr(  if last.lbstunit; )";
        put "%nrstr(run; )";
        put "%nrstr( )";
        put "%nrstr(data _lb10tscg_4;)";
        put "%nrstr(  set _lb10tscg_3; )";
        put "%nrstr(  attrib)";
        put "%nrstr(    basevar   length=8  label=%"Baseline Variable%")";
        put "%nrstr(    normlvar  length=8  label=%"Normalized Variable%")";
        put "%nrstr(    ;)";
        put "%nrstr(  retain) &g_centid &g_subjid &keepvar &g_trtcd &g_trtgrp %nrstr(lbtestcd lbtest lbstunit basevar normlvar; )";
        put "%nrstr(run;)";
        put " ";         
        %if "&dataset"^="" %then %do;
          put "%nrstr(data) &dataset%nrstr(;)";
          put "%nrstr(  set _lb10tscg_4; )";
          put "%nrstr(run; )";
          put "%nrstr( )";
        %end;
        put "%nrstr(%mend; )";
        put "%nrstr( )";
        put "%nrstr(%td_lb10tscg_preprocess; )";
        put "%nrstr( )";
      run;    
