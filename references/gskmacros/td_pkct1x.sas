...
|                    identified in the IDSL Data Display Standards. (Cross Over Study)
|
...
|
| COLUMNS           Passed as %tu_sumstatsincol's parameter of the same name   &g_ptrtcd &g_ptrtgrp
...
|
| GROUPBYVARPOP     Passed as %tu_sumstatsincol's parameter of the same name   &g_ptrtcd (Req)
|
| GROUPBYVARSANALY  Passed as %tu_sumstatsincol's parameter of the same name   &g_ptrtcd &g_ptrtgrp visitnum visit
|                                                                               ptmnum ptm no_imputed (Req)
|
| IDVARS            Passed as %tu_sumstatsincol's parameter of the same name   &g_ptrtcd &g_ptrtgrp
...
|
| NIMPUTEVARS       Specifies the levels of classification for calculating     &g_ptrtcd visitnum ptmnum (Req)
|                   the number of imputed values
|
| NOPRINTVARS       Passed as %tu_sumstatsincol's parameter of the same name   &g_ptrtcd visitnum ptmnum (Req)
|
...
|
| ORDERVARS         Passed as %tu_sumstatsincol's parameter of the same name   &g_ptrtcd &g_ptrtgrp
|                                                                              tt_bnnm visitnum visit ptmnum ptm (Req)
|
...
|
| WIDTHS            Passed as %tu_sumstatsincol's parameter of the same name   &g_ptrtgrp 15 visit 10
|                                                                              ptm 10 (Req)
|
|---------------------------------------------------------------------------------------------------
|
...
|
| Modified By: Trevor Welby
| Date of Modification: 08-Mar-05
| New version/draft number: 01-003
| Modification ID: TQW9753.01-003
| Reason For Modification: Remove the WHERE clause, macro now subsets the input
...
|
|                          Create a version for Cross Over Design Study
|
...
%macro td_pkct1x(
  ...
  COLUMNS                 =&g_ptrtcd &g_ptrtgrp tt_bnnm visitnum visit ptmnum ptm n no_imputed mean std median min max,  /* Columns to be included in the listing (plus spanned headers) */
  ...
  GROUPBYVARSANALY        =%nrstr(&g_ptrtcd &g_ptrtgrp visitnum visit ptmnum ptm no_imputed),  /* The variables whose values define the subgroup combinations for the analysis */
  IDVARS                  =&g_ptrtcd &g_ptrtgrp tt_bnnm visitnum visit ptmnum ptm,  /* Variables to appear on each page of the report */
  ...
  NIMPUTEVARS             =&g_ptrtcd visitnum ptmnum,  /* Class variables used to calculate the number imputed */
  NOPRINTVARS             =&g_ptrtcd visitnum ptmnum,  /* No print variables, used to order the display */
  ...
  ORDERVARS               =&g_ptrtcd &g_ptrtgrp tt_bnnm visitnum visit ptmnum ptm ,  /* Order variables */
  ...
  WIDTHS                  =&g_ptrtgrp 15 visit 10 ptm 10  /* Column widths */
  );
  ...
  /*
  / Verify NIMPUTEVARS exists on the DSETIN dataset
  /------------------------------------------------------------------------------*/
  %local nonexistvars;
