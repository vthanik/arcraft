...
| Date:               17Sep2007
|
| Macro purpose:      The unit will produce a Listing of Liver Event Results and Time of 
|                     Event Relative to Treatment
|
...
|                     PROC REPORT COLUMN statement syntax, but     rutrt1st rutrtst  
|                     not including report_item=alias syntax       ruorrscd ruorres  
|                                                                                    
...
|                     dataset                                      new  listing      
|                     Valid values: a non-blank text string        LIVER5            
|                                                                                    
...
|                     widths for all variables not listed in the   rutrt1st 12       
|                     WIDTHS parameter.                            rutrtst 10        
|                     Valid values: values of column names and     ruorres 20        
...
|                     Valid values: name of an existing dataset,   =(rutestcd eq     
|                     pre-sorted by BYVARS                         'LVCHMEVT'))      
|                                                                                    
| FLOWVARS            Variables to be defined with the flow        st_all_cs         
|                     option.                                      st_all_asr        
|                     Valid values: one or more variable names     st_liver_dsd5     
|                     from DSETIN that are also defined with       ruorres           
|                     COLUMNS                                                        
...
|                     COLUMNS                                                        
|                                                                                    
| LABELS              Variables and their label for display. For   ruorres='Result   
|                     use where label for display differs to the   Exceeding         
|                     label on the DSETIN                          Stopping Criteria'
...
|                     statement attribute noprint.                 ruorrscd          
|                     Valid values: one or more variable names                       
|                     from DSETIN that are also defined with                         
|                     COLUMNS                                                        
...
%macro td_liver5(
   ...
   COLUMNS             =&g_centid &g_subjid st_all_cs st_all_asr rudt st_liver_dsd5 rutrt1st rutrtst ruorrscd ruorres, /* Column parameter */
   ...
   DDDATASETLABEL      =DD dataset for new  listing LIVER5, /* Label to be applied to the DD dataset */
   DEFAULTWIDTHS       =st_all_cs 7 st_all_asr 25 st_liver_dsd5 10 rutrt1st 12 rutrtst 10 ruorres 20, /* List of default column widths */
   DESCENDING          =,                  /* Descending ORDERVARS */
   DSETIN              =ardata.rucam(where=(rutestcd eq 'LVCHMEVT')),/* Input dataset */
   FLOWVARS            =st_all_cs st_all_asr st_liver_dsd5 ruorres, /* Variables with flow option */
   FORMATS             =,                  /* Format specification */
   IDVARS              =,                  /* Variables to appear on each page should the report be wider than 1 page */
   LABELS              =ruorres='Result Exceeding Stopping Criteria', /* Label definitions. */
   LEFTVARS            =,                  /* Left justify variables */
   LINEVARS            =,                  /* Order variable printed with line statements. */
   NOPRINTVARS         =&g_centid &g_subjid rudt ruorrscd, /* No print vars (usually used to order the display) */
   ...
   ORDERVARS           =&g_centid &g_subjid st_all_cs st_all_asr rudt st_liver_dsd5 rutrt1st rutrtst ruorrscd, /* Order variables */
