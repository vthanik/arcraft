...
| Date:               18May2007
|
| Macro Purpose:      The macro produces a summary table. The number and percentage of
|                     subjects who failed screening and therefore were not entered
|                     into the study will be summarized by reason for failure.
|
...
|                     tu_freq or tu_sumstatsinrows and             rdata.ds(where=(ds
|                     appropriate parameters for the macro to      scatcd in (2))),
|                     create an output dataset. The percentage     groupByVarsNumer=
|                     sign shall not be used as a prefix to the    &g_trtcd dsfail,
|                     macro name, and the parameters shall not be  DSETOUT=segment1_
|                     surrounded by brackets, i.e. the following   out(rename=(dsfail
|                     style shall         be used:                 =tt_code1)),ROWLAB
|                     segment1=MacroName parm1=value1,             ELVARNAME=tt_grpla
|                     parm2=value2                                 bel='Screening 
|                     Valid values: A complete call to tu_freq or  Status')
|                     ru_sumstatsinrows
|
...
|                     from DSETIN plus other elements of valid     tt_ac999
...
|                     DD macro.  This parameter specifies column   tt_ac999 20
...
|                     immediately prior to creation of the         tt_segorder eq 1
|                     permanent presentation dataset. Used for     then
|                     subsetting records required for computation  tt_decode1=put(tt_
|                     but not for display.                         code1, $dsfails.))
...
|
| XMLDEFAULTS         Specifies a XML file which defined           &g_refdata/tr_es6_
...
| Date of Modification:     05-Nov-2007
| New version number:       1/2
| Modification ID:          n/a
| Reason For Modification:  Change default label of tt_decode1 to '~' in
|                           default values of LABELS, based on UAT results
|-----------------------------------------------------------------------------
| Modified By:
...
%macro td_es6 (
   SEGMENT1            =%str(dsetinnumer=ardata.ds(where=(dsscatcd in (2))), groupByVarsNumer=&g_trtcd dsfail,DSETOUT=segment1_out(rename=(dsfail=tt_code1)),ROWLABELVARNAME=tt_grplabel='Screening Status'),/* Same as SEGMENT3 */
   SEGMENT2            =%str(dsetinnumer=ardata.ds(where=(dsscatcd in (2) and dsfail eq 'Y')),groupByVarsNumer=&g_trtcd dssbrscd, DSETOUT=segment2_out(rename=(dssbrscd=tt_code1 dssbrs=tt_decode1))),/* Same as SEGMENT3 */
   ...
   COLUMNS             =tt_segorder tt_grplabel tt_code1 tt_decode1 tt_ac999, /* Column parameter */
   ...
   DDDATASETLABEL      =DD dataset for new ES6 table, /* Label to be applied to the DD dataset */
   DEFAULTWIDTHS       =tt_decode1 45 tt_grplabel 45 tt_ac999 20, /* List of default column widths */
   ...
   POSTSUBSET          =%nrstr(if tt_segorder eq 1 then tt_decode1=put(tt_code1, $dsfails.)), /* SAS expression to be applied to data immediately prior to creation of the permanent presentation dataset */
   ...
   XMLDEFAULTS         =&g_refdata/tr_es6_defaults.xml /* Location and name of XML defaults file */
   );
   ...
   %if &l_flag %then
   %do;
      libname _tdes6 xml "&xmldefaults";
      proc sql noprint;
         select count(memname) into :l_flag
         from dictionary.tables
         where libname eq '_TDES6' and memname eq 'ES6'
         ;
      quit; 
      %if not &l_flag %then
      %do;      
         libname _tdes6 clear;
      %end;      
   %end;
