# Enrollment template specification — Table 14.1.1

spec_enrollment <- function(data = NULL) {
  default_vars <- c("SITEID", "COUNTRY", "REGION1")
  avail_vars <- if (!is.null(data)) intersect(default_vars, names(data)) else default_vars
  if (length(avail_vars) == 0) avail_vars <- "SITEID"
  ...
  list(
    ...
    fmt = list(
      titles = list(
        ...
        list(text = "Subject Enrollment by Site", bold = FALSE)
      ),
