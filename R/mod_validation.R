...
mod_validation_ui <- function(id) {
  ...
  htmltools::tagList(
    htmltools::tags$button(
      ...
      htmltools::tags$i(class = "fa fa-check-circle", style = "margin-right: 6px;"),
      "Run Validation"
    ),
...
mod_validation_server <- function(id, store) {
    ...
    output$checklist <- shiny::renderUI({
      ...
      htmltools::tagList(
        htmltools::tags$div(class = "ar-flex ar-items-center ar-gap-8 ar-mb-12",
          htmltools::tags$span(style = "font-size: 13px; font-weight: 600;",
                               paste0(n_pass, "/", n_total, " checks passed")),
          if (n_pass == n_total) {
            htmltools::tags$span(style = "color: var(--success); font-size: 12px;", "Ready to export")
          }
        ),
